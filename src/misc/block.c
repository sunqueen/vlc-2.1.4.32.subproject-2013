...
void block_Init( block_t *restrict b, void *buf, size_t size )
{
    ...
    b->p_next = NULL;
    b->p_buffer = (uint8_t *)buf;			// sunqueen modify
    b->i_buffer = size;
    b->p_start = (uint8_t *)buf;			// sunqueen modify
    b->i_size = size;
    b->i_flags = 0;
    b->i_nb_samples = 0;
    b->i_pts =
...
block_t *block_Alloc (size_t size)
{
    ...
    block_t *b = (block_t *)malloc (alloc);			// sunqueen modify
    if (unlikely(b == NULL))
        return NULL;
    ...
    b->p_buffer = (uint8_t *)(((uintptr_t)b->p_buffer) & ~(BLOCK_ALIGN - 1));			// sunqueen modify
    b->i_buffer = size;
...
{
    block_t *block = (block_t *)malloc (sizeof (*block));				// sunqueen modify
    if (block == NULL)
    {
        free (addr);
        return NULL;
...
{
    block_fifo_t *p_fifo = (block_fifo_t *)malloc( sizeof( block_fifo_t ) );			// sunqueen modify
    if( !p_fifo )
        return NULL;
