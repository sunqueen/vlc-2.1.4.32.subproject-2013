...
{
    subpicture_t *p_subpic = (subpicture_t *)calloc( 1, sizeof(*p_subpic) );			// sunqueen modify
    if( !p_subpic )
        return NULL;
    ...
    if( p_upd )
    {
        subpicture_private_t *p_private = (subpicture_private_t *)malloc( sizeof(*p_private) );			// sunqueen modify
        if( !p_private )
        {
            free( p_subpic );
            return NULL;
...
{
    subpicture_region_private_t *p_private = (subpicture_region_private_t *)malloc( sizeof(*p_private) );			// sunqueen modify
    ...
    if( p_fmt->p_palette )
    {
        p_private->fmt.p_palette = (video_palette_t *)malloc( sizeof(*p_private->fmt.p_palette) );			// sunqueen modify
