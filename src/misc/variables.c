...
{
    const variable_t *va = (const variable_t *)a, *vb = (const variable_t *)b;				// sunqueen modify
...
int var_Create( vlc_object_t *p_this, const char *psz_name, int i_type )
{
    ...
    variable_t *p_var = (variable_t *)calloc( 1, sizeof( *p_var ) );				// sunqueen modify
    if( p_var == NULL )
        return VLC_ENOMEM;
...
{
    Destroy( (variable_t *)var );				// sunqueen modify
}
                ...
                int i_action, vlc_value_t *p_val, vlc_value_t *p_val2 )
{
    ...
    switch( i_action )
    {
        ...
        case VLC_VAR_GETLIST:
            p_val->p_list = (vlc_list_t *)malloc( sizeof(vlc_list_t) );			// sunqueen modify
            if( p_val2 ) p_val2->p_list = (vlc_list_t *)malloc( sizeof(vlc_list_t) );			// sunqueen modify
            if( p_var->choices.i_count )
            {
                p_val->p_list->p_values = (vlc_value_t *)malloc( p_var->choices.i_count
                                                  * sizeof(vlc_value_t) );			// sunqueen modify
                p_val->p_list->pi_types = (int *)malloc( p_var->choices.i_count
                                                  * sizeof(int) );			// sunqueen modify
                if( p_val2 )
                {
                    p_val2->p_list->p_values = (vlc_value_t *)
                        malloc( p_var->choices.i_count * sizeof(vlc_value_t) );			// sunqueen modify
                    p_val2->p_list->pi_types = (int *)
                        malloc( p_var->choices.i_count * sizeof(int) );			// sunqueen modify
                }
            }
...
int var_LocationParse (vlc_object_t *obj, const char *mrl, const char *pref)
{
    ...
    while (*mrl != '\0')
    {
        ...
        char *buf = (char *)malloc (preflen + len);			// sunqueen modify
