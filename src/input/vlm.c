                               ...
                               vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    preparse_data_t *p_pre = (preparse_data_t *)p_data;				// sunqueen modify
                       ...
                       void *p_data )
{
    ...
    assert( p_vlm );
    vlm_media_sys_t *p_media = (vlm_media_sys_t *)p_data;			// sunqueen modify
    ...
    if( newval.i_int == INPUT_EVENT_STATE )
    {
        ...
        vlm_SendEventMediaInstanceState( p_vlm, p_media->cfg.id, p_media->cfg.psz_name, psz_instance_name, (input_state_e)var_GetInteger( p_input, "state" ) );			// sunqueen modify
...
vlm_t *vlm_New ( vlc_object_t *p_this )
{
    ...
    p_vlm = (vlm_t *)vlc_custom_create( p_this->p_libvlc, sizeof( *p_vlm ),			// sunqueen modify
                               "vlm daemon" );
    if( !p_vlm )
    {
        vlc_mutex_unlock( (vlc_mutex_t *)&vlm_mutex );			// sunqueen modify
        return NULL;
    }
...
static int vlm_ControlMediaAdd( vlm_t *p_vlm, vlm_media_t *p_cfg, int64_t *p_id )
{
    ...
    if( p_cfg->b_vod && !p_vlm->p_vod )
    {
        p_vlm->p_vod = (vod_t *)vlc_custom_create( VLC_OBJECT(p_vlm), sizeof( vod_t ),				// sunqueen modify
                                          "vod server" );
    ...
    p_media = (vlm_media_sys_t *)calloc( 1, sizeof( vlm_media_sys_t ) );			// sunqueen modify
    if( !p_media )
        return VLC_ENOMEM;
...
{
    vlm_media_instance_sys_t *p_instance = (vlm_media_instance_sys_t *)calloc( 1, sizeof(vlm_media_instance_sys_t) );			// sunqueen modify
    if( !p_instance )
        return NULL;
    ...
    p_instance->p_parent = (vlc_object_t *)vlc_object_create( p_vlm, sizeof (vlc_object_t) );			// sunqueen modify
