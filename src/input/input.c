                               ...
                               input_resource_t *p_resource )
{
    ...
    /* Allocate descriptor */
    p_input = (input_thread_t *)vlc_custom_create( p_parent, sizeof( *p_input ), "input" );			// sunqueen modify
    if( p_input == NULL )
        return NULL;
    ...
    p_input->p = (input_thread_private_t *)calloc( 1, sizeof( input_thread_private_t ) );			// sunqueen modify
    if( !p_input->p )
        return NULL;
...
static void InitPrograms( input_thread_t * p_input )
{
    ...
    if( p_input->p->p_sout )
    {
        ...
        if( (prgms = var_GetNonEmptyString( p_input, "programs" )) != NULL )
        {
            ...
            {
                vlc_value_t val = { /*.i_int =*/ atoi( prgm ) };			// sunqueen modify
...
static int Init( input_thread_t * p_input )
{
    ...
    if( !p_input->b_preparsing )
    {
        ...
        {
            vlc_value_t val = { /*.i_int =*/ INPUT_RATE_DEFAULT / f_rate };			// sunqueen modify
...
static input_source_t *InputSourceNew( input_thread_t *p_input )
{
    ...
    return (input_source_t *)calloc( 1,  sizeof( input_source_t ) );			// sunqueen modify
}
                             ...
                             input_source_t *p_source, vlc_meta_t *p_meta )
{
    ...
    demux_meta_t *p_demux_meta = (demux_meta_t *)			// sunqueen modify
...
static void InputMetaUser( input_thread_t *p_input, vlc_meta_t *p_meta )
{
    ...
    for( int i = 0; p_list[i].psz_name; i++ )
    {
        ...
        vlc_meta_Set( p_meta, (vlc_meta_type_t)p_list[i].i_meta, psz_string );			// sunqueen modify
        free( psz_string );
    }
}
                              ...
                              int i_new, input_attachment_t **pp_new )
{
    ...
    attachment = (input_attachment_t **)xrealloc( attachment,			// sunqueen modify
