...
{
    es_out_t     *out = (es_out_t *)malloc( sizeof( *out ) );			// sunqueen modify
    if( !out )
        return NULL;

    es_out_sys_t *p_sys = (es_out_sys_t *)calloc( 1, sizeof( *p_sys ) );			// sunqueen modify
    if( !p_sys )
    {
        free( out );
        return NULL;
...
static es_out_pgrm_t *EsOutProgramAdd( es_out_t *out, int i_group )
{
    ...
    es_out_pgrm_t *p_pgrm = (es_out_pgrm_t *)malloc( sizeof( es_out_pgrm_t ) );			// sunqueen modify
    if( !p_pgrm )
        return NULL;
...
static es_out_id_t *EsOutAdd( es_out_t *out, const es_format_t *fmt )
{
    ...
    es_out_id_t   *es = (es_out_id_t *)malloc( sizeof( *es ) );			// sunqueen modify
    es_out_pgrm_t *p_pgrm;
    int i;
...
static int EsOutControlLocked( es_out_t *out, int i_query, va_list args )
{
    ...
    case ES_OUT_SET_GROUP_META:
    {
        ...
        const vlc_meta_t *p_meta = (vlc_meta_t*)va_arg( args, const vlc_meta_t * );			// sunqueen modify
    ...
    {
        int i_group = (int)va_arg( args, int );
        const vlc_epg_t *p_epg = (vlc_epg_t*)va_arg( args, const vlc_epg_t * );			// sunqueen modify
    ...
    case ES_OUT_GET_ES_OBJECTS_BY_ID:
    {
        ...
        vlc_object_t    **pp_decoder = (vlc_object_t **)va_arg( args, vlc_object_t ** );			// sunqueen modify
        vout_thread_t   **pp_vout    = (vout_thread_t **)va_arg( args, vout_thread_t ** );			// sunqueen modify
        audio_output_t **pp_aout    = (audio_output_t **)va_arg( args, audio_output_t ** );			// sunqueen modify
        if( p_es->p_dec )
        {
            if( pp_decoder )
                *pp_decoder = (vlc_object_t *)vlc_object_hold( p_es->p_dec );			// sunqueen modify
    ...
    {
        bool *pb = (bool*)va_arg( args, bool* );			// sunqueen modify
