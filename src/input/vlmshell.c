...
static int ExecuteLoad( vlm_t *p_vlm, const char *psz_path, vlm_message_t **pp_status )
{
    ...
    psz_buffer = (char *)malloc( i_size + 1 );			// sunqueen modify
    if( !psz_buffer )
    {
        stream_Delete( p_stream );
                                    ...
                                    const int i_property, char *ppsz_property[], vlm_message_t **pp_status )
{
    ...
    for( i = 0; i < i_property; i++ )
    {
        ...
        else if( !strcmp( ppsz_property[i], "append" ) )
        {
            ...
            {
                psz_line = (char *)xrealloc( psz_line,			// sunqueen modify
                           ...
                           vlm_message_t **pp_message )
{
    ...
    char *buf = (char *)malloc( i_command_len + 1 ), *psz_buf = buf;			// sunqueen modify
    size_t i_ppsz_command_len = (3 + (i_command_len + 1) / 2);
    char **ppsz_command = (char **)malloc( i_ppsz_command_len * sizeof(char *) );			// sunqueen modify
...
static vlm_schedule_sys_t *vlm_ScheduleNew( vlm_t *vlm, const char *psz_name )
{
    ...
    vlm_schedule_sys_t *p_sched = (vlm_schedule_sys_t *)malloc( sizeof( vlm_schedule_sys_t ) );			// sunqueen modify
    if( !p_sched )
        return NULL;
...
vlm_message_t *vlm_MessageSimpleNew( const char *psz_name )
{
    ...
    vlm_message_t *p_message = (vlm_message_t *)malloc( sizeof(*p_message) );			// sunqueen modify
    if( !p_message )
        return NULL;
...
static char *Save( vlm_t *vlm )
{
    ...
    p = save = (char *)malloc( i_length );			// sunqueen modify
    if( !save ) return NULL;
    *save = '\0';
