...
static void cmd_cleanup_routine( ts_cmd_t *p ) { CmdClean( p ); }				// sunqueen modify
...
es_out_t *input_EsOutTimeshiftNew( input_thread_t *p_input, es_out_t *p_next_out, int i_rate )
{
    ...
    es_out_sys_t *p_sys = (es_out_sys_t *)malloc( sizeof(*p_sys) );			// sunqueen modify
    if( !p_sys )
    {
        free( p_out );
        return NULL;
...
static es_out_id_t *Add( es_out_t *p_out, const es_format_t *p_fmt )
{
    ...
    es_out_id_t *p_es = (es_out_id_t *)malloc( sizeof( *p_es ) );			// sunqueen modify
    if( !p_es )
        return NULL;
...
static int TsStart( es_out_t *p_out )
{
    ...
    p_sys->p_ts = p_ts = (ts_thread_t *)calloc(1, sizeof(*p_ts));			// sunqueen modify
    if( !p_ts )
        return VLC_EGENERIC;
...
{
    ts_thread_t *p_ts = (ts_thread_t *)p_data;			// sunqueen modify
...
//                vlc_value_t val = { .i_int = p_ts->i_rate };
         ...
         * reading  */
        vlc_cleanup_push( (void (__cdecl *)(void *))cmd_cleanup_routine, &cmd );				// sunqueen modify
...
{
    ts_storage_t *p_storage = (ts_storage_t *)calloc( 1, sizeof(ts_storage_t) );			// sunqueen modify
    if( !p_storage )
        return NULL;
    ...
    p_storage->p_cmd = (ts_cmd_t *)malloc( p_storage->i_cmd_max * sizeof(*p_storage->p_cmd) );			// sunqueen modify
...
static void TsStoragePack( ts_storage_t *p_storage )
{
    ...
    ts_cmd_t *p_new = (ts_cmd_t *)realloc( p_storage->p_cmd, p_storage->i_cmd_max * sizeof(*p_storage->p_cmd) );			// sunqueen modify
    if( p_new )
...
static int CmdInitAdd( ts_cmd_t *p_cmd, es_out_id_t *p_es, const es_format_t *p_fmt, bool b_copy )
{
    ...
    if( b_copy )
    {
        p_cmd->u.add.p_fmt = (es_format_t *)malloc( sizeof(*p_fmt) );			// sunqueen modify
...
static int CmdInitControl( ts_cmd_t *p_cmd, int i_query, va_list args, bool b_copy )
{
    ...
    case ES_OUT_SET_ES_FMT:     /* arg1= es_out_id_t* arg2=es_format_t* */
    {
        ...
        if( b_copy )
        {
            p_cmd->u.control.u.es_fmt.p_fmt = (es_format_t *)malloc( sizeof(*p_fmt) );			// sunqueen modify
...
static char *GetTmpPath( char *psz_path )
{
    ...
    wchar_t *psw_path = (wchar_t *)calloc( dwCount + 1, sizeof(wchar_t) );			// sunqueen modify
    if( psw_path )
    {
