                              ...
                              vout_thread_t **pp_vout, audio_output_t **pp_aout )
{
    ...
    vlc_mutex_lock( &p_owner->lock );
    if( pp_vout )
        *pp_vout = p_owner->p_vout ? (vout_thread_t *)vlc_object_hold( p_owner->p_vout ) : NULL;				// sunqueen modify
    if( pp_aout )
        *pp_aout = p_owner->p_aout ? (audio_output_t *)vlc_object_hold( p_owner->p_aout ) : NULL;				// sunqueen modify
    vlc_mutex_unlock( &p_owner->lock );
}
                                  ...
                                  sout_instance_t *p_sout )
{
    ...
    p_dec = (decoder_t *)vlc_custom_create( p_parent, sizeof( *p_dec ), "decoder" );				// sunqueen modify
    if( p_dec == NULL )
        return NULL;
    ...
    p_dec->p_owner = p_owner = (decoder_owner_sys_t *)malloc( sizeof( decoder_owner_sys_t ) );			// sunqueen modify
    ...
    {
        p_owner->p_packetizer = (decoder_t *)				// sunqueen modify
...
{
    decoder_t *p_dec = (decoder_t *)p_private;			// sunqueen modify
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    input_thread_t *p_input = p_owner->p_input;
