...
stream_t *stream_CommonNew( vlc_object_t *p_obj )
{
    ...
    s->p_text = (stream_text_t *)malloc( sizeof(*s->p_text) );			// sunqueen modify
    if( !s->p_text )
    {
...
stream_t *stream_AccessNew( access_t *p_access, char **ppsz_list )
{
    ...
    {
        access_entry_t *p_entry = (access_entry_t *)malloc( sizeof(*p_entry) );			// sunqueen modify
        if( !p_entry )
            goto error;
        ...
        for( int i = 0; ppsz_list[i] != NULL; i++ )
        {
            ...
            p_entry = (access_entry_t *)malloc( sizeof(*p_entry) );			// sunqueen modify
            if( p_entry )
            {
    ...
    else
    {
        ...
        p_sys->stream.i_tk     = 0;
        p_sys->stream.p_buffer = (uint8_t *)malloc( STREAM_CACHE_SIZE );			// sunqueen modify
...
static int AStreamReadBlock( stream_t *s, void *p_read, unsigned int i_read )
{
    ...
    uint8_t *p_data = (uint8_t *)p_read;			// sunqueen modify
...
static int AStreamPeekBlock( stream_t *s, const uint8_t **pp_peek, unsigned int i_read )
{
    ...
    {
        p_sys->p_peek = (uint8_t *)realloc_or_free( p_sys->p_peek, i_read );			// sunqueen modify
        if( !p_sys->p_peek )
        {
            p_sys->i_peek = 0;
            return 0;
...
char *stream_ReadLine( stream_t *s )
{
    ...
    for( ;; )
    {
        ...
        {
            /* UTF-8: 0A <LF> */
            psz_eol = (char *)memchr( p_data, '\n', i_data );				// sunqueen modify
            if( psz_eol == NULL )
                /* UTF-8: 0D <CR> */
                psz_eol = (char *)memchr( p_data, '\r', i_data );				// sunqueen modify
        }
        else
        {
        ...
        if( psz_eol )
        {
            ...
            p_line = (char *)realloc_or_free( p_line,
                     i_line + i_data + s->p_text->i_char_width ); /* add \0 */				// sunqueen modify
            if( !p_line )
                goto error;
        ...
        p_line = (char *)realloc_or_free( p_line,
                       i_line + STREAM_PROBE_LINE + s->p_text->i_char_width );			// sunqueen modify
        if( !p_line )
            goto error;
    ...
    if( i_read > 0 )
    {
        ...
        if( s->p_text->i_char_width > 1 )
        {
            ...
            psz_new_line = (char *)malloc( i_new_line );			// sunqueen modify
...
static int AReadStream( stream_t *s, void *p_read, unsigned int i_read )
{
    ...
    if( !p_sys->i_list )
    {
        i_read = p_access->pf_read( p_access, (uint8_t *)p_read, i_read );			// sunqueen modify
    ...
    i_read = p_sys->p_list_access->pf_read( p_sys->p_list_access, (uint8_t *)p_read,			// sunqueen modify
                                            i_read );
