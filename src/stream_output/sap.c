...
sap_handler_t *SAP_Create (vlc_object_t *p_announce)
{
    ...
    p_sap = (sap_handler_t *)vlc_custom_create (p_announce, sizeof (*p_sap), "sap sender");			// sunqueen modify
    if (p_sap == NULL)
        return NULL;
...
static sap_address_t *AddressCreate (vlc_object_t *obj, const char *group)
{
    ...
    sap_address_t *addr = (sap_address_t *)malloc (sizeof (*addr));			// sunqueen modify
    if (addr == NULL)
    {
        net_Close (fd);
        return NULL;
...
{
    sap_address_t *addr = (sap_address_t *)self;			// sunqueen modify
    ...
    for (;;)
    {
        ...
        {
            send (addr->fd, (const char *)p_session->data, p_session->length, 0);			// sunqueen modify
...
int SAP_Add (sap_handler_t *p_sap, session_descriptor_t *p_session)
{
    ...
    p_sap_session = (sap_session_t *)malloc (sizeof (*p_sap_session) + length + 1);			// sunqueen modify
