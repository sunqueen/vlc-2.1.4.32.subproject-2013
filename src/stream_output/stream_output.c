...
sout_instance_t *sout_NewInstance( vlc_object_t *p_parent, const char *psz_dest )
{
    ...
    /* *** Allocate descriptor *** */
    p_sout = (sout_instance_t *)vlc_custom_create( p_parent, sizeof( *p_sout ), "stream output" );			// sunqueen modify
    if( p_sout == NULL )
    {
        free( psz_chain );
        return NULL;
                                        ...
                                        es_format_t *p_fmt )
{
    ...
    p_input         = (sout_packetizer_input_t *)malloc( sizeof( sout_packetizer_input_t ) );			// sunqueen modify
                                      ...
                                      const char *psz_access, const char *psz_name )
{
    ...
    p_access = (sout_access_out_t *)vlc_custom_create( p_sout, sizeof( *p_access ), "access out" );			// sunqueen modify
    if( !p_access )
        return NULL;
                          ...
                          sout_access_out_t *p_access )
{
    ...
    p_mux = (sout_mux_t *)vlc_custom_create( p_sout, sizeof( *p_mux ), "mux" );			// sunqueen modify
    if( p_mux == NULL )
        return NULL;
...
sout_input_t *sout_MuxAddStream( sout_mux_t *p_mux, es_format_t *p_fmt )
{
    ...
    p_input = (sout_input_t *)malloc( sizeof( sout_input_t ) );			// sunqueen modify
    if( !p_input )
        return NULL;
    p_input->p_fmt  = p_fmt;
                               ...
                               config_chain_t *p_cfg, sout_stream_t *p_next)
{
    ...
    p_stream = (sout_stream_t *)vlc_custom_create( p_sout, sizeof( *p_stream ), "stream out" );			// sunqueen modify
    if( !p_stream )
        return NULL;
                                ...
                                sout_stream_t *p_next, sout_stream_t **pp_last)
{
    ...
    while(i--)
    {
        p_next = sout_StreamNew( p_sout, (char *)vlc_array_item_at_index(&name, i),
            (config_chain_t *)vlc_array_item_at_index(&cfg, i), p_next);			// sunqueen modify
...
error:
    ...
    while(modules--)
        sout_StreamDelete((sout_stream_t *)vlc_array_item_at_index(&module, modules));			// sunqueen modify
    vlc_array_clear(&module);
    ...
    while(i--)
    {
        ...
        config_ChainDestroy((config_chain_t *)vlc_array_item_at_index(&cfg, i));			// sunqueen modify
    }
    vlc_array_clear(&name);
    vlc_array_clear(&cfg);
...
{
    return (encoder_t *)vlc_custom_create( p_this, sizeof( encoder_t ), "encoder" );			// sunqueen modify
}
