...
{
    size_t count;
    vlc_sd_probe_t *tab = (vlc_sd_probe_t *)vlc_probe (obj, "services probe", &count);			// sunqueen modify
    ...
    char **names = (char **)malloc (sizeof(char *) * (count + 1));			// sunqueen modify
    char **longnames = (char **)malloc (sizeof(char *) * (count + 1));			// sunqueen modify
    int *categories = (int *)malloc(sizeof(int) * (count + 1));			// sunqueen modify
                                     ...
                                     const char *cfg )
{
    ...
    p_sd = (services_discovery_t *)vlc_custom_create( p_super, sizeof( *p_sd ), "services discovery" );			// sunqueen modify
    if( !p_sd )
        return NULL;
...
bool vlc_sd_Start ( services_discovery_t * p_sd )
{
    ...
    vlc_event_t event = {
        /*.type =*/ vlc_ServicesDiscoveryStarted			// sunqueen modify
    };
...
static void playlist_sd_item_added( const vlc_event_t * p_event, void * user_data )
{
    ...
    playlist_item_t * p_parent = (playlist_item_t *)user_data;			// sunqueen modify
    playlist_t * p_playlist = (playlist_t *)p_parent->p_playlist;			// sunqueen modify
...
static void playlist_sd_item_removed( const vlc_event_t * p_event, void * user_data )
{
    ...
    playlist_item_t * p_sd_node = (playlist_item_t *)user_data;			// sunqueen modify
                                   ...
                                   const char *psz_name )
{
    ...
    vlc_sd_internal_t * p_sds = (vlc_sd_internal_t *)malloc( sizeof(*p_sds) );			// sunqueen modify
    if( !p_sds )
    {
        vlc_sd_Destroy( p_sd );
        return VLC_ENOMEM;
