...
{
    playlist_fetcher_t *p_fetcher = (playlist_fetcher_t *)malloc( sizeof(*p_fetcher) );			// sunqueen modify
    if( !p_fetcher )
        return NULL;
...
static int FindArt( playlist_fetcher_t *p_fetcher, input_item_t *p_item )
{
    ...
    art_finder_t *p_finder = (art_finder_t *)			// sunqueen modify
...
static int DownloadArt( playlist_fetcher_t *p_fetcher, input_item_t *p_item )
{
    ...
    for( ;; )
    {
        ...
        p_data = (uint8_t *)realloc_or_free( p_data, i_data + i_read );			// sunqueen modify
        if( !p_data )
            break;
...
{
    demux_meta_t *p_demux_meta = (demux_meta_t *)vlc_custom_create(p_fetcher->object,			// sunqueen modify
...
{
    playlist_fetcher_t *p_fetcher = (playlist_fetcher_t *)p_data;			// sunqueen modify
