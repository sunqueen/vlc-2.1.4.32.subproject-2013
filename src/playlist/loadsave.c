                     ...
                     playlist_item_t *p_export_root, const char *psz_type )
{
    ...
    playlist_export_t *p_export = (playlist_export_t *)
        vlc_custom_create( p_playlist, sizeof( *p_export ), "playlist export" );			// sunqueen modify
    if( !p_export )
        return VLC_ENOMEM;
...
static void input_item_subitem_tree_added( const vlc_event_t * p_event,
                                      void * user_data )
{
    playlist_t *p_playlist = (playlist_t *)user_data;			// sunqueen modify
