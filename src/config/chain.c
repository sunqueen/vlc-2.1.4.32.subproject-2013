...
const char *config_ChainParseOptions( config_chain_t **pp_cfg, const char *psz_opts )
{
    ...
    do
    {
        ...
        config_chain_t *p_cfg = (config_chain_t *)malloc( sizeof(*p_cfg) );			// sunqueen modify
        if( !p_cfg )
            break;
...
config_chain_t *config_ChainDuplicate( const config_chain_t *p_src )
{
    ...
    {
        config_chain_t *p = (config_chain_t *)malloc( sizeof(*p) );				// sunqueen modify
        if( !p )
            break;
        p->p_next    = NULL;
...
char *config_StringEscape( const char *str )
{
    ...
    char *ret = (char *)xmalloc( length + 1 ), *dst = ret;			// sunqueen modify
    for( const char *p = str; *p; p++ )
    {
        if( IsEscapeNeeded( *p ) )
            *dst++ = '\\';
