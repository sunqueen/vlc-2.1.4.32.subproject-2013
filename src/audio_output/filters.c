...
{
    filter_t *filter = (filter_t *)vlc_custom_create (obj, sizeof (*filter), type);			// sunqueen modify
                                        ...
                                        video_format_t *fmt)
{
    ...
    const aout_request_vout_t *req = (aout_request_vout_t *)filter->p_owner;			// sunqueen modify
                        ...
                        const audio_sample_format_t *restrict outfmt)
{
    ...
    filter_t *filter = CreateFilter (obj, type, name,
                                     (filter_owner_sys_t *)owner, infmt, outfmt);			// sunqueen modify
    if (filter == NULL)
    {
...
{
    aout_filters_t *filters = (aout_filters_t *)malloc (sizeof (*filters));			// sunqueen modify
