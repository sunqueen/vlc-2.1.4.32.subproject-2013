...
static int  FilterAllocationInit(filter_t *filter, void *vd)
{
    ...
    filter->p_owner             = (filter_owner_sys_t *)vd;			// sunqueen modify
...
{
    /* */
    vout_display_t *vd = (vout_display_t *)vlc_custom_create(obj, sizeof(*vd), "vout display" );			// sunqueen modify
...
{
    vout_display_owner_sys_t *osys = (vout_display_owner_sys_t *)data;			// sunqueen modify
...
{
    /* */
    vout_display_owner_sys_t *osys = (vout_display_owner_sys_t *)calloc(1, sizeof(*osys));			// sunqueen modify
    vout_display_cfg_t *cfg = &osys->cfg;
                                 ...
                                 mtime_t hide_timeout)
{
    ...
    vout_display_sys_t *sys = (vout_display_sys_t *)malloc(sizeof(*sys));			// sunqueen modify
    if (!sys)
        abort();
    sys->picture = (picture_t **)calloc(splitter->i_output, sizeof(*sys->picture));			// sunqueen modify
    if (!sys->picture )
        abort();
    ...
    /* */
    video_splitter_owner_t *owner = (video_splitter_owner_t *)malloc(sizeof(*owner));			// sunqueen modify
    if (!owner)
        abort();
