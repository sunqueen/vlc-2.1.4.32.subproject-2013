...
int vlc_mkstemp( char *_template )			// sunqueen modify
{
    static const char digits[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    static const int i_digits = sizeof(digits)/sizeof(*digits) - 1;

    /* */
    assert( _template );			// sunqueen modify

    /* Check template validity */
    const size_t i_length = strlen( _template );			// sunqueen modify
    char *psz_rand = &_template[i_length-6];			// sunqueen modify
    ...
    for( int i = 0; i < 256; i++ )
    {
        ...
        /* */
        int fd = vlc_open( _template, O_CREAT | O_EXCL | O_RDWR, 0600 );			// sunqueen modify
        if( fd >= 0 )
            return fd;
        if( errno != EEXIST )
            return -1;
