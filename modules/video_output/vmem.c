...
static int Open(vlc_object_t *object)
{
    ...
    /* Get the callbacks */
    vlc_format_cb setup = (vlc_format_cb)var_InheritAddress(vd, "vmem-setup");			// sunqueen modify

    sys->lock = (void *(__cdecl *)(void *,void **))var_InheritAddress(vd, "vmem-lock");			// sunqueen modify
    if (sys->lock == NULL) {
        msg_Err(vd, "missing lock callback");
        free(sys);
        return VLC_EGENERIC;
    }
    sys->unlock = (void (__cdecl *)(void *,void *,void *const *))var_InheritAddress(vd, "vmem-unlock");			// sunqueen modify
    sys->display = (void (__cdecl *)(void *,void *))var_InheritAddress(vd, "vmem-display");			// sunqueen modify
    sys->cleanup = (void (__cdecl *)(void *))var_InheritAddress(vd, "vmem-cleanup");			// sunqueen modify
...
static int Lock(picture_t *picture)
{
    ...
    for (int i = 0; i < picture->i_planes; i++)
        picture->p[i].p_pixels = (uint8_t *)planes[i];			// sunqueen modify
