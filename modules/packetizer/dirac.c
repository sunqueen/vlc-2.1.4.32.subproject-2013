...
struct dirac_reorder_buffer {
    int u_size_max;
    int u_size;
    struct dirac_reorder_entry p_entries[32], *p_head, *p_empty;
...
struct seq_hdr_t {
    ...
    uint32_t u_fps_num;
    uint32_t u_fps_den;
    enum picture_coding_mode_t u_picture_coding_mode;
} ; /*< sequence header */
// sunqueen move add end
...
#endif
	// sunqueen move delete end
	struct dirac_reorder_buffer reorder_buf;			// sunqueen modify
...
#endif
	// sunqueen move delete end
	struct seq_hdr_t seq_hdr;			// sunqueen modify
...
static dirac_block_encap_t *dirac_RemoveBlockEncap( block_t *p_block )
{
    ...
    dirac_block_encap_t *p_dbe = (dirac_block_encap_t *)p_fake->p_dbe;			// sunqueen modify
...
static void dirac_AddBlockEncap( block_t **pp_block, dirac_block_encap_t *p_dbe )
{
    ...
    fake_block_t *p_fake = (fake_block_t *)xcalloc( 1, sizeof( *p_fake ) );			// sunqueen modify
...
{
    dirac_block_encap_t *p_dbe = (dirac_block_encap_t *)calloc( 1, sizeof( *p_dbe ) );			// sunqueen modify
...
static int block_ChainToArray( block_t *p_block, block_t ***ppp_array)
{
    ...
    *ppp_array = (block_t **)calloc( i_num_blocks, sizeof( block_t* ) );			// sunqueen modify
...
static bool dirac_UnpackSeqHdr( struct seq_hdr_t *p_sh, block_t *p_block )
{
    ...
    p_sh->u_picture_coding_mode = (picture_coding_mode_t)dirac_uint( &bs );			// sunqueen modify
...
static int Open( vlc_object_t *p_this )
{
    ...
    p_dec->p_sys = p_sys = (decoder_sys_t *)calloc( 1, sizeof( decoder_sys_t ) );			// sunqueen modify
