 ...
 * $Id: a8193a7cb04aebedc59241694e31f75c74ddde43 $
 *
...
static int Open( vlc_object_t *p_this )
{
    ...
    if( ( p_dec->p_sys = p_sys = (decoder_sys_t *)malloc( sizeof(decoder_sys_t) ) ) == NULL )			// sunqueen modify
    {
        return VLC_ENOMEM;
    }
    ...
    if( p_dec->fmt_in.i_original_fourcc == VLC_FOURCC( 'a', 'v', 'c', '1' ) )
    {
        ...
        {
            uint8_t *p_dst = (uint8_t *)p_dec->fmt_out.p_extra;			// sunqueen modify
    ...
    else
    {
        ...
        if( p_dec->fmt_in.i_extra > 0 )
            packetizer_Header( &p_sys->packetizer,
                               (const uint8_t *)p_dec->fmt_in.p_extra, p_dec->fmt_in.i_extra );			// sunqueen modify
    }
                              ...
                              const uint8_t *src, int i_src )
{
    ...
    uint8_t *dst = (uint8_t *)malloc( i_src );			// sunqueen modify
