...
static int OpenDemux( vlc_object_t *p_this )
{
    ...
    for( i_len = 0, psz_sdp = NULL; i_len < 65536; )
    {
        ...
        char *psz_sdp_new = (char *)realloc( psz_sdp, i_len + i_read_max + 1 );			// sunqueen modify
        size_t i_read;
        if( psz_sdp_new == NULL )
        {
            errval = VLC_ENOMEM;
...
{
    services_discovery_t *p_sd = (services_discovery_t *)data;			// sunqueen modify
    char *psz_addr;
    int i;
    int timeout = -1;
    ...
    if (l != NULL)
    {
        ...
        if( s != INVALID_SOCKET )
        {
            ...
            if( SOCKET_ERROR != WSAIoctl(s, SIO_GET_INTERFACE_LIST, NULL, 0, &ifaces, len, (LPDWORD)&len, NULL, NULL) )			// sunqueen modify
            {
...
static int Demux( demux_t *p_demux )
{
    ...
    p_input = (input_thread_t *)demux_GetParentInput( p_demux );			// sunqueen modify
    assert( p_input );
    if( !p_input )
    {
                     ...
                     size_t len )
{
    ...
    if( b_compressed )
    {
        ...
        decomp = (uint8_t *)realloc (decomp, newsize + 1);			// sunqueen modify
        decomp[newsize] = '\0';
...
static sdp_t *ParseSDP (vlc_object_t *p_obj, const char *psz_sdp)
{
    ...
    sdp_t *p_sdp = (sdp_t *)calloc (1, sizeof (*p_sdp));			// sunqueen modify
    if (p_sdp == NULL)
        return NULL;
...
//        char line[linelen + 1];
        ...
        switch (expect)
        {
            ...
            media:
            {
                ...
                m = (sdp_media_t *)realloc (p_sdp->mediav, (p_sdp->mediac + 1) * sizeof (*m));			// sunqueen modify
                if (m == NULL)
                {
					free (line);			// sunqueen add
                    goto error;
...
static inline attribute_t *MakeAttribute (const char *str)
{
    attribute_t *a = (attribute_t *)malloc (sizeof (*a) + strlen (str) + 1);			// sunqueen modify
    if (a == NULL)
        return NULL;
