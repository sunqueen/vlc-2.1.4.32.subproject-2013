 ...
 * $Id: b5aee73f1d5895f11e25dd37cb70fd91de260e94 $
 *
...
static char *ConstructUrl( const char *_template, const char *base_url,			// sunqueen modify
        const uint64_t bandwidth, const uint64_t start_time )
{
    char *frag, *end, *qual;
    char *url_template = strdup( _template );
    char *saveptr = NULL;
...
static chunk_t * chunk_Get( sms_stream_t *sms, const int64_t start_time )
{
    ...
    for( int i = 0; i < len; i++ )
    {
        chunk_t * chunk = (chunk_t *)vlc_array_item_at_index( sms->chunks, i );			// sunqueen modify
...
static int sms_Download( stream_t *s, chunk_t *chunk, char *url )
{
    ...
    chunk->data = (uint8_t *)malloc( size );			// sunqueen modify
    ...
    if( read < size )
    {
        ...
        chunk->data = (uint8_t *)realloc( chunk->data, read );			// sunqueen modify
    }
...
static int get_new_chunks( stream_t *s, chunk_t *ck )
{
    ...
    tfrf_df = (TfrfBoxDataFields_t *)calloc( fragment_count, sizeof( TfrfBoxDataFields_t ) );			// sunqueen modify
...
{
    chunk_t *ret = (chunk_t *)calloc( 1, sizeof( chunk_t ) );			// sunqueen modify
    if( unlikely( ret == NULL ) )
        goto build_init_chunk_error;

    ret->size = SMOO_SIZE;
    ret->data = (uint8_t *)malloc( SMOO_SIZE );			// sunqueen modify
    if( !ret->data )
        goto build_init_chunk_error;
...
void* sms_Thread( void *p_this )
{
    ...
    for( int i = 0; i < 3; i++ )
    {
        ...
        if( sms )
        {
            chunk = (chunk_t *)vlc_array_item_at_index( sms->chunks, 0 );			// sunqueen modify
    ...
    while( 1 )
    {
        ...
        if( p_sys->b_tseek )
        {
            ...
            {
                ck = (chunk_t *)vlc_array_item_at_index( p_sys->download.chunks, i );			// sunqueen modify
                if( unlikely( !ck ) )
                    goto cancel;
                ck->read_pos = 0;
                if( ck->data == NULL )
