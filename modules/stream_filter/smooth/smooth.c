...
static bool isSmoothStreaming( stream_t *s )
{
    ...
    char *peeked = (char *)malloc( 512 );			// sunqueen modify
    if( unlikely( !peeked ) )
        return false;
...
static int Open( vlc_object_t *p_this )
{
    ...
    s->p_sys = p_sys = (stream_sys_t *)calloc( 1, sizeof(*p_sys ) );			// sunqueen modify
    ...
    *pos = '\0';
    ...
    {
        tmp = (sms_stream_t *)vlc_array_item_at_index( p_sys->sms_streams, i );			// sunqueen modify
    ...
    {
        wanted = qlvl = NULL;
        sms = (sms_stream_t *)vlc_array_item_at_index( p_sys->sms_streams, i );			// sunqueen modify
        wanted = (quality_level_t *)vlc_array_item_at_index( sms->qlevels, 0 );			// sunqueen modify
        for( unsigned i=1; i < sms->qlevel_nb; i++ )
        {
            qlvl = (quality_level_t *)vlc_array_item_at_index( sms->qlevels, i );			// sunqueen modify
...
static void Close( vlc_object_t *p_this )
{
    ...
    {
        chunk = (chunk_t *)vlc_array_item_at_index( p_sys->init_chunks, i );			// sunqueen modify
        chunk_Free( chunk );
    }
...
static chunk_t *get_chunk( stream_t *s, const bool wait )
{
    ...
    chunk = (chunk_t *)vlc_array_item_at_index( p_sys->download.chunks, p_sys->playback.index );			// sunqueen modify
