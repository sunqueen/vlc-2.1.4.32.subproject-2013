...
{ \
    ...
    const type *p_src = (type *) p_srcorig; \
    type *p_dest = (type *) p_destorig; \
 \
...
static int OpenFilter( vlc_object_t *p_this )
{
    ...
    for( uint8_t in_ch = 0, wg4_i = 0; in_ch < audio_in->i_channels; in_ch++, wg4_i++ )
    {
        ...
        uint8_t *pi_chnidx = (uint8_t *) memchr( channel_wg4idx, wg4_i, channel_wg4idx_len );			// sunqueen modify
    ...
    for( uint8_t i = 0; i < audio_in->i_channels; i++ )
    {
        ...
        uint8_t *pi_out_ch = (uint8_t *) memchr( out_ch_sorted, wg4_out_ch, audio_out->i_channels );			// sunqueen modify
