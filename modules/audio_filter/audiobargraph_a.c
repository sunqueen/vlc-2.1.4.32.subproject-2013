 ...
 * $Id: 53223d3a76d498ee22d701becb496b2e4f780df6 $
 *
...
static block_t *DoWork( filter_t *p_filter, block_t *p_in_buf )
{
    ...
    char message[255];
    int nbChannels = 0;
    ValueDate_t* _new = NULL;			// sunqueen modify
    ...
    if (p_sys->silence) {
        ...
        _new = (ValueDate_t*)malloc(sizeof(ValueDate_t));
        _new->value = max;
        _new->date = p_in_buf->i_pts;
        _new->next = NULL;
        if (p_sys->last != NULL) {
            p_sys->last->next = _new;
        }
        p_sys->last = _new;
        if (p_sys->first == NULL) {
            p_sys->first = _new;
        ...
        while (p_sys->first->date < (_new->date - (p_sys->time_window*1000))) {			// sunqueen modify
