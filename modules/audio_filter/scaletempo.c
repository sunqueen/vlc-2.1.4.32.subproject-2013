...
static unsigned best_overlap_offset_float( filter_t *p_filter )
{
    ...
    pw  = (float *)p->table_window;			// sunqueen modify
    po  = (float *)p->buf_overlap;			// sunqueen modify
    po += p->samples_per_frame;
    ppc = (float *)p->buf_pre_corr;			// sunqueen modify
...
{
    filter_sys_t *p = p_filter->p_sys;
    float *pout = (float *)buf_out;			// sunqueen modify
    float *pb   = (float *)p->table_blend;			// sunqueen modify
    float *po   = (float *)p->buf_overlap;			// sunqueen modify
...
static int reinit_buffers( filter_t *p_filter )
{
    ...
    else
    {
        ...
        if( ! p->buf_pre_corr || ! p->table_window )
            return VLC_ENOMEM;
        float *pw = (float *)p->table_window;			// sunqueen modify
    ...
    p->buf_queue = (uint8_t *)malloc( p->bytes_queue_max );			// sunqueen modify
    if( ! p->buf_queue )
        return VLC_ENOMEM;
...
static int Open( vlc_object_t *p_this )
{
    ...
    /* Allocate structure */
    filter_sys_t *p_sys = p_filter->p_sys = (filter_sys_t *)malloc( sizeof(*p_sys) );			// sunqueen modify
    if( ! p_sys )
        return VLC_ENOMEM;
