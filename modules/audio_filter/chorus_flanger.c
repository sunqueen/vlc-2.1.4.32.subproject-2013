...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys->p_delayLineStart = (float *)calloc( p_sys->i_bufferLength, sizeof( float ) );			// sunqueen modify
...
static int reallocate_buffer( filter_t *p_filter,  filter_sys_t *p_sys )
{
    ...
    float *temp = (float *)realloc( p_sys->p_delayLineStart, p_sys->i_bufferLength );			// sunqueen modify
    if( unlikely( !temp ) )
    {
