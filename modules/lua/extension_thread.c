...
int Activate( extensions_manager_t *p_mgr, extension_t *p_ext )
{
    ...
    p_sys->command = (command_t *)calloc( 1, sizeof( struct command_t ) );			// sunqueen modify
    if( !p_sys->command )
        return VLC_ENOMEM;
...
int Deactivate( extensions_manager_t *p_mgr, extension_t *p_ext )
{
    ...
    /* Push command */
    struct command_t *cmd = (command_t *)calloc( 1, sizeof( struct command_t ) );			// sunqueen modify
                   ...
                   va_list args )
{
    ...
    switch( i_command )
    {
        ...
        case CMD_TRIGGERMENU:
            {
                int *pi = (int *)malloc( sizeof( int ) );			// sunqueen modify
                if( !pi )
                {
                    free( cmd );
                    vlc_mutex_unlock( &p_ext->p_sys->command_lock );
    ...
    while( !p_ext->p_sys->b_exiting )
    {
        ...
        if( LockExtension( p_ext ) )
        {
            switch( cmd->i_command )
            {
                ...
                case CMD_CLICK:
                {
                    extension_widget_t *p_widget = (extension_widget_t *)cmd->data[0];			// sunqueen modify
                    assert( p_widget );
                ...
                case CMD_TRIGGERMENU:
                {
                    int *pi_id = (int *)cmd->data[0];			// sunqueen modify
                    assert( pi_id );
