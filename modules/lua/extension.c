                     ...
                     void *dummy )
{
    ...
    if( !strncasecmp( psz_filename + i_flen - 4, ".vle", 4 ) )
    {
        ...
        psz_script = (char *)calloc( 1, i_flen + 6 + 12 + 1 );			// sunqueen modify
        if( !psz_script )
            return 0;
    ...
    if( lua_gettop( L ) )
    {
        if( lua_istable( L, -1 ) )
        {
            ...
            if( !lua_isnil( L, -1 ) && lua_isstring( L, -1 ) )
            {
                ...
                p_ext->p_icondata = (char *)malloc( len );			// sunqueen modify
                if( p_ext->p_icondata )
                {
...
static int Control( extensions_manager_t *p_mgr, int i_control, va_list args )
{
    ...
    switch( i_control )
    {
        ...
        case EXTENSION_SET_INPUT:
        {
            ...
            p_ext->p_sys->p_input = p_input ? (input_thread_t *)vlc_object_hold( p_input )			// sunqueen modify
                                            : p_input;
                           ...
                           const char *psz_function, va_list args )
{
    ...
    while( ( type = (lua_datatype_e)va_arg( args, int ) ) != LUA_END )			// sunqueen modify
    {
        if( type == LUA_NUM )
        {
                                             ...
                                             void *p_data )
{
    ...
    extension_dialog_command_t *command = (extension_dialog_command_t *)newval.p_address;			// sunqueen modify
    assert( command != NULL );
    assert( command->p_dlg != NULL);

    extension_t *p_ext = (extension_t *)command->p_dlg->p_sys;			// sunqueen modify
    assert( p_ext != NULL );

    extension_widget_t *p_widget = (extension_widget_t *)command->p_data;			// sunqueen modify
...
{
    extension_t *p_ext = (extension_t *)data;			// sunqueen modify
