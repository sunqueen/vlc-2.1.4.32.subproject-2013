...
static input_item_t* vlclua_input_item_get_internal( lua_State *L )
{
    input_item_t **pp_item = (input_item_t **)luaL_checkudata( L, 1, "input_item" );			// sunqueen modify
...
static int vlclua_input_item_get( lua_State *L, input_item_t *p_item )
{
    ...
    input_item_t **pp = (input_item_t **)lua_newuserdata( L, sizeof( input_item_t* ) );			// sunqueen modify
    *pp = p_item;
...
#undef META_TYPE
    ...
    for( unsigned i = 0; i < VLC_META_TYPE_COUNT; i++ )
    {
        ...
        {
            type = (vlc_meta_type_t)pp_meta_types[i].type;			// sunqueen modify
