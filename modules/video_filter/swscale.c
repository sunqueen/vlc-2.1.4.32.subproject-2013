...
static int OpenScaler( vlc_object_t *p_this )
{
    ...
    if( ( p_filter->p_sys = p_sys = (filter_sys_t *)malloc(sizeof(filter_sys_t)) ) == NULL )			// sunqueen modify
        return VLC_ENOMEM;
...
static int Init( filter_t *p_filter )
{
    ...
    for( int n = 0; n < (cfg.b_has_a ? 2 : 1); n++ )
    {
        ...
        ctx = sws_getContext( i_fmti_width, p_fmti->i_height, (PixelFormat)i_fmti,
                              i_fmto_width, p_fmto->i_height, (PixelFormat)i_fmto,
                              cfg.i_sws_flags | p_sys->i_cpu_mask,
                              p_sys->p_src_filter, p_sys->p_dst_filter, 0 );			// sunqueen modify
        if( n == 0 )
            p_sys->ctx = ctx;
        else
            p_sys->ctxA = ctx;
