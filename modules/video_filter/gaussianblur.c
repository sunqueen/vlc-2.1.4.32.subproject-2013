...
static void gaussianblur_InitDistribution( filter_sys_t *p_sys )
{
    ...
    type_t *pt_distribution = (type_t *)xmalloc( (2*i_dim+1) * sizeof( type_t ) );			// sunqueen modify
    int x;
...
static picture_t *Filter( filter_t *p_filter, picture_t *p_pic )
{
    ...
    if( !p_sys->pt_buffer )
    {
        p_sys->pt_buffer = (int *)realloc_or_free( p_sys->pt_buffer,			// sunqueen modify
    ...
    if( !p_sys->pt_scale )
    {
        ...
        p_sys->pt_scale = (int *)xmalloc( i_visible_lines * i_pitch * sizeof( type_t ) );			// sunqueen modify
