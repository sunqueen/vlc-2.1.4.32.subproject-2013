...
static int Open( vlc_object_t *p_this )
{
    ...
    /* Allocate structure */
    p_splitter->p_sys = p_sys = (video_splitter_sys_t *)malloc( sizeof( *p_sys ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
    ...
    p_splitter->i_output =
        Configuration( p_sys->pp_output,
                       ...
                       p_sys->b_attenuate,
                       pb_active );
    p_splitter->p_output = (video_splitter_output_t *)calloc( p_splitter->i_output,			// sunqueen modify
...
static double GammaFactor( const panoramix_gamma_t *g, float f_value )
{
    if( f_value <= g->f_black_crush )
    {
        ...
        return pow( (double)f_input, 1.0 / g->f_gamma );			// sunqueen modify
    }
