                                      ...
                                      char *psz_offsets )
{
    if( *psz_offsets )
    {
        ...
        do
        {
            ...
            p_sys->pi_x_offsets = (int *)xrealloc( p_sys->pi_x_offsets,			// sunqueen modify
                                                   i_index * sizeof(int) );
            p_sys->pi_x_offsets[i_index - 1] = atoi( psz_offsets );
            psz_end = strchr( psz_offsets, ',' );
            psz_offsets = psz_end + 1;

            p_sys->pi_y_offsets = (int *)xrealloc( p_sys->pi_y_offsets,			// sunqueen modify
                                                   i_index * sizeof(int) );
...
#undef GET_VAR
    ...
    if( *psz_order )
    {
        ...
        do
        {
            ...
            i_index++;
            p_sys->ppsz_order = (char **)xrealloc( p_sys->ppsz_order,			// sunqueen modify
                                                 i_index * sizeof(char *) );
