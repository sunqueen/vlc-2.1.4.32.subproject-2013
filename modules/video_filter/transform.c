...
{ \
    const uint##bits##_t *src_pixels = (const uint##bits##_t *)src->p_pixels; \
    uint##bits##_t *restrict dst_pixels = (uint##bits##_t *)dst->p_pixels; \
...
static int Open(vlc_object_t *object)
{
    ...
    filter_sys_t *sys = (filter_sys_t *)malloc(sizeof(*sys));			// sunqueen modify
    if (!sys)
        return VLC_ENOMEM;
