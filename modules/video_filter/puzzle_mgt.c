 ...
 * $Id: b6a986213130c8a3e0bacc0eadc0a020108e812a $
 *
...
int puzzle_generate_rand_pce_list( filter_t *p_filter, int32_t **pi_pce_lst )
{
    ...
    if (pi_pce_lst != NULL )
        free( *pi_pce_lst );
    *pi_pce_lst = (int32_t *)calloc( i_pieces_nbr, sizeof(**pi_pce_lst) );			// sunqueen modify
    if( !*pi_pce_lst )
        return VLC_ENOMEM;
...
int puzzle_piece_foreground( filter_t *p_filter, int32_t i_piece) {
    ...
    ps_pieces_tmp = (piece_t *)malloc( sizeof( piece_t) * p_sys->s_allocated.i_pieces_nbr );			// sunqueen modify
    if (!ps_pieces_tmp)
        return VLC_ENOMEM;
...
save_game_t* puzzle_save(filter_t *p_filter)
{
    ...
    save_game_t *ps_save_game = (save_game_t *)calloc(1, sizeof(*ps_save_game));			// sunqueen modify
    if (!ps_save_game)
        return NULL;
    ...
    ps_save_game->ps_pieces = (save_piece_t *)calloc( ps_save_game->i_cols * ps_save_game->i_rows , sizeof(*ps_save_game->ps_pieces));			// sunqueen modify
