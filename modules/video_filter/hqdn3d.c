 ...
 * Open
 *****************************************************************************/
static int Open(vlc_object_t *_this)			// sunqueen modify
{
    filter_t *filter = (filter_t *)_this;			// sunqueen modify
    filter_sys_t *sys;
    struct vf_priv_s *cfg;
    ...
    /* Allocate structure */
    sys = (filter_sys_t *)calloc(1, sizeof(filter_sys_t));			// sunqueen modify
    if (!sys) {
        return VLC_ENOMEM;
    }
    cfg = &sys->cfg;
    ...
    cfg->Line = (unsigned int *)malloc(wmax*sizeof(int));			// sunqueen modify
    if (!cfg->Line) {
        free(sys);
        return VLC_ENOMEM;
    }
 ...
 * Close
 *****************************************************************************/
static void Close(vlc_object_t *_this)			// sunqueen modify
{
    filter_t *filter = (filter_t *)_this;			// sunqueen modify
