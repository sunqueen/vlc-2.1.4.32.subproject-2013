...
static int CreateFilter( vlc_object_t *p_this )
{
    ...
    p_sys->psz_marquee = (char *)malloc( p_sys->i_length + 1 );			// sunqueen modify
...
static int ParseUrls( filter_t *p_filter, char *psz_urls )
{
    ...
    p_sys->p_feeds = (rss_feed_t *)malloc( p_sys->i_feeds * sizeof( rss_feed_t ) );			// sunqueen modify
    if( !p_sys->p_feeds )
        return VLC_ENOMEM;
    ...
    while( (type = xml_ReaderNextNode( p_xml_reader, &node )) > 0 )
    {
        ...
        case XML_READER_STARTELEM:
            ...
            if( !strcmp( node, "item" ) || !strcmp( node, "entry" ) )
            {
                b_is_item = true;
                p_feed->i_items++;
                p_feed->p_items = (rss_item_t *)xrealloc( p_feed->p_items,			// sunqueen modify
