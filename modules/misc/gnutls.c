...
{
    vlc_tls_t *session = (vlc_tls_t *)opaque;			// sunqueen modify
...
static int gnutls_SessionOpen (vlc_tls_creds_t *crd, vlc_tls_t *session,
                               int type, int fd)
{
	vlc_tls_sys_t *sys = (vlc_tls_sys_t *)malloc(sizeof (*session->sys));			// sunqueen modify
...
static int gnutls_AddCA (vlc_tls_creds_t *crd, const char *path)
{
    ...
    gnutls_datum_t d = {
       /*.data =*/ block->p_buffer,			// sunqueen modify
	   /*.size =*/ block->i_buffer,			// sunqueen modify
    };
...
static int OpenServer (vlc_tls_creds_t *crd, const char *cert, const char *key)
{
	...
	vlc_tls_creds_sys_t *sys = (vlc_tls_creds_sys_t *)malloc (sizeof (*sys));			// sunqueen modify
    if (unlikely(sys == NULL))
        goto error;
    ...
    gnutls_datum_t pub = {
       /*.data =*/ certblock->p_buffer,			// sunqueen modify
	   /*.size =*/ certblock->i_buffer,			// sunqueen modify
    }, priv = {
       /*.data =*/ keyblock->p_buffer,			// sunqueen modify
	   /*.size =*/ keyblock->i_buffer,			// sunqueen modify
    };
    ...
    {
        const gnutls_datum_t data = {
			/*.data =*/ (unsigned char *)dh_params,			// sunqueen modify
			/*.size =*/ sizeof (dh_params)-1,			// sunqueen modify
        };
