...
static void parse_recordings( vlc_object_t *p_obj, json_value *node, acoustid_result_t *p_result )
{
    ...
    p_result->recordings.p_recordings = (musicbrainz_recording_t *)calloc( node->u.array.length, sizeof(musicbrainz_recording_t) );			// sunqueen modify
...
static bool ParseJson( vlc_object_t *p_obj, char *psz_buffer, acoustid_results_t *p_results )
{
    ...
    p_results->p_results = (acoustid_result_t *)calloc( node->u.array.length, sizeof(acoustid_result_t) );			// sunqueen modify
...
int DoAcoustIdWebRequest( vlc_object_t *p_obj, acoustid_fingerprint_t *p_data )
{
    ...
    for( ;; )
    {
        ...
        request.p_buffer = (char *)realloc_or_free( request.p_buffer, 1 + i_ret + i_read );			// sunqueen modify
        if( !request.p_buffer )
        {
            i_status = VLC_ENOMEM;
            goto cleanup;
