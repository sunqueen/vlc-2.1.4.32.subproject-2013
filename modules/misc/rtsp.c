...
static int Open( vlc_object_t *p_this )
{
    ...
    p_vod->p_sys = p_sys = (vod_sys_t *)malloc( sizeof( vod_sys_t ) );			// sunqueen modify
    if( !p_sys ) goto error;
    p_sys->p_rtsp_host = 0;
                              ...
                              input_item_t *p_item )
{
    ...
    vod_media_t *p_media = (vod_media_t *)calloc( 1, sizeof(vod_media_t) );			// sunqueen modify
    if( !p_media )
        return NULL;
    ...
    httpd_UrlCatch( p_media->p_rtsp_url, HTTPD_MSG_SETUP,
                    RtspCallback, (httpd_callback_sys_t *)p_media );			// sunqueen modify
...
static int MediaAddES( vod_t *p_vod, vod_media_t *p_media, es_format_t *p_fmt )
{
    ...
    media_es_t *p_es = (media_es_t *)calloc( 1, sizeof(media_es_t) );			// sunqueen modify
    if( !p_es )
        return VLC_ENOMEM;
    ...
    switch( p_fmt->i_codec )
    {
        ...
        case VLC_CODEC_H264:
            ...
            if( p_fmt->i_extra > 0 )
            {
                uint8_t *p_buffer = (uint8_t *)p_fmt->p_extra;			// sunqueen modify
        ...
        case VLC_CODEC_MP4V:
            ...
            if( p_fmt->i_extra > 0 )
            {
                char *p_hexa = (char *)malloc( 2 * p_fmt->i_extra + 1 );			// sunqueen modify
                sprintf_hexa( p_hexa, (uint8_t *)p_fmt->p_extra, p_fmt->i_extra );			// sunqueen modify
                if( asprintf( &p_es->psz_fmtp,
    ...
    httpd_UrlCatch( p_es->p_rtsp_url, HTTPD_MSG_SETUP,
                    RtspCallbackES, (httpd_callback_sys_t *)p_es );			// sunqueen modify
...
{
    rtsp_client_t *p_rtsp = (rtsp_client_t *)calloc( 1, sizeof(rtsp_client_t) );			// sunqueen modify
...
//#warning Session ID should be securely random (spoofing risk)			// sunqueen delete
                ...
                p_rtsp_es = (rtsp_client_es_t *)malloc( sizeof(rtsp_client_es_t) );			// sunqueen modify
                if( !p_rtsp_es )
                {
                    answer->i_status = 500;
                    answer->i_body = 0;
