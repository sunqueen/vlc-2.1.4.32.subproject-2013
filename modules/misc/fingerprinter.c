...
static fingerprint_request_t * GetResult( fingerprinter_thread_t *f )
{
    ...
    {
        r = (fingerprint_request_t *)vlc_array_item_at_index( p_sys->results.queue, 0 );			// sunqueen modify
...
static int Open(vlc_object_t *p_this)
{
    ...
    fingerprinter_sys_t *p_sys = (fingerprinter_sys_t *)calloc(1, sizeof(fingerprinter_sys_t));			// sunqueen modify
...
static void Close(vlc_object_t *p_this)
{
    ...
    for ( int i = 0; i < vlc_array_count( p_sys->incoming.queue ); i++ )
        fingerprint_request_Delete( (fingerprint_request_t *)vlc_array_item_at_index( p_sys->incoming.queue, i ) );			// sunqueen modify
    ...
    for ( int i = 0; i < vlc_array_count( p_sys->processing.queue ); i++ )
        fingerprint_request_Delete( (fingerprint_request_t *)vlc_array_item_at_index( p_sys->processing.queue, i ) );			// sunqueen modify
    ...
    for ( int i = 0; i < vlc_array_count( p_sys->results.queue ); i++ )
        fingerprint_request_Delete( (fingerprint_request_t *)vlc_array_item_at_index( p_sys->results.queue, i ) );			// sunqueen modify
...
//**
        ...
        {
            fingerprint_request_t *p_data = (fingerprint_request_t *)vlc_array_item_at_index( p_sys->processing.queue, p_sys->i );			// sunqueen modify
