...
static int Open( vlc_object_t *p_this )
{
    ...
    if( unlikely( !( p_sys = (sout_access_out_sys_t *)malloc ( sizeof( *p_sys ) ) ) ) )			// sunqueen modify
        return VLC_ENOMEM;
...
static int LoadCryptFile( sout_access_out_t *p_access )
{
    ...
    //First read key_uri
	size_t size = 0;			// sunqueen add
    ssize_t len = getline( &key_uri, &size/*&(size_t){0}*/, stream );			// sunqueen modify
    if( unlikely( len == -1 ) )
    {
    ...
    len = getline( &key_file, &size/*&(size_t){0}*/, stream );
    if( unlikely( len == -1 ) )
    {
        msg_Err( p_access, "Cannot read %s: %m", p_sys->psz_keyfile );
        clearerr( stream );
...
static uint32_t segmentAmountNeeded( sout_access_out_sys_t *p_sys )
{
    ...
    {
        output_segment_t* segment = (output_segment_t *)vlc_array_item_at_index( p_sys->segments_t, vlc_array_count( p_sys->segments_t ) - index );			// sunqueen modify
...
static bool isFirstItemRemovable( sout_access_out_sys_t *p_sys, uint32_t i_firstseg, uint32_t i_index_offset )
{
    ...
    {
        output_segment_t *segment = (output_segment_t *)vlc_array_item_at_index( p_sys->segments_t, p_sys->i_segment - i_firstseg + index );			// sunqueen modify
        duration += segment->f_seglength;
    }
    output_segment_t *first = (output_segment_t *)vlc_array_item_at_index( p_sys->segments_t, 0 );			// sunqueen modify
...
static int updateIndexAndDel( sout_access_out_t *p_access, sout_access_out_sys_t *p_sys, bool b_isend )
{
    ...
    {
         output_segment_t *segment = (output_segment_t *)vlc_array_item_at_index( p_sys->segments_t, 0 );			// sunqueen modify
