...
static int LoadFontsFromAttachments( filter_t *p_filter )
{
    ...
    p_sys->pp_font_attachments = (input_attachment_t **)malloc( i_attachments_cnt * sizeof(*p_sys->pp_font_attachments));			// sunqueen modify
...
static int GetFileFontByName( LPCTSTR font_name, char **psz_filename )
{
	...
	char *font_name_temp = FromT((const wchar_t *)font_name);			// sunqueen modify
    ...
    for( int index = 0;; index++ )
    {
		...
		char *psz_value = FromT((const wchar_t *)vbuffer);			// sunqueen modify
	...
	*psz_filename = FromT((const wchar_t *)dbuffer);			// sunqueen modify
    free( font_name_temp );
    RegCloseKey( hKey );
    return 0;
}
                           ...
                           bool b_bold, bool b_italic, int i_size, int *i_idx )
{
    ...
    LPTSTR psz_fbuffer = (LPTSTR)ToT( family );			// sunqueen modify
	_tcsncpy((wchar_t *)&lf.lfFaceName, psz_fbuffer, LF_FACESIZE);
    free( psz_fbuffer );
                       ...
                       FT_BBox *p_bbox )
{
    ...
    if( !p_region->p_picture )
        return VLC_EGENERIC;
    fmt.p_palette = p_region->fmt.p_palette ? p_region->fmt.p_palette : (video_palette_t *)malloc(sizeof(*fmt.p_palette));			// sunqueen modify
    p_region->fmt = fmt;
                     ...
                     uint32_t i_color, uint32_t i_karaoke_bg_color )
{
    ...
    font_stack_t *p_new = (font_stack_t *)malloc( sizeof(*p_new) );			// sunqueen modify
    if( !p_new )
        return VLC_ENOMEM;
                           ...
                           uint32_t i_k_date )
{
    ...
    uni_char_t *psz_tmp = (uni_char_t *)ToCharset( FREETYPE_TO_UCS, psz_text_in, &i_string_bytes );			// sunqueen modify
    if( psz_tmp )
    {
...
{
    line_desc_t *p_line = (line_desc_t *)malloc( sizeof(*p_line) );			// sunqueen modify
    ...
    p_line->p_character = (line_character_t *)calloc( i_count, sizeof(*p_line->p_character) );			// sunqueen modify
...
static FT_Face LoadEmbeddedFace( filter_sys_t *p_sys, const text_style_t *p_style )
{
    for( int k = 0; k < p_sys->i_font_attachments; k++ )
    {
        ...
        while( 0 == FT_New_Memory_Face( p_sys->p_library,
                                        (const FT_Byte *)p_attach->p_data,			// sunqueen modify
                                        p_attach->i_data,
                                        i_font_idx,
                                        &p_face ))
        {
                         ...
                         int i_len )
{
    ...
    if( pi_k_dates )
    {
        pi_karaoke_bar = (uint8_t *)malloc( i_len * sizeof(*pi_karaoke_bar));			// sunqueen modify
        if( pi_karaoke_bar )
        {
...
#define SAVE_BP(dst) do { \
        ...
        while( i_index < i_start + i_length )
        {
            ...
            while( i_part_length > 0 )
            {
                ...
                /* Get kerning vector */
                FT_Vector kerning = { /*.x =*/ 0, /*.y =*/ 0 };			// sunqueen modify
...
#endif
                p_line->p_character[p_line->i_character_count].p_glyph = (FT_BitmapGlyph)glyph;
                p_line->p_character[p_line->i_character_count].p_outline = (FT_BitmapGlyph)outline;
                p_line->p_character[p_line->i_character_count].p_shadow = (FT_BitmapGlyph)shadow;
                p_line->p_character[p_line->i_character_count].i_color = i_color;
                p_line->p_character[p_line->i_character_count].i_line_offset = i_line_offset;
                p_line->p_character[p_line->i_character_count].i_line_thickness = i_line_thickness;
                         ...
                         const vlc_fourcc_t *p_chroma_list )
{
    ...
    uni_char_t *psz_text = (uni_char_t *)calloc( i_text_max, sizeof( *psz_text ) );			// sunqueen modify
    text_style_t **pp_styles = (text_style_t **)calloc( i_text_max, sizeof( *pp_styles ) );			// sunqueen modify
    ...
    if( b_html )
    {
        ...
        if( !rv )
        {
            ...
            if( xml_ReaderNextNode( p_xml_reader, &node ) == XML_READER_STARTELEM )
            {
                ...
                {
                    pi_k_durations = (uint32_t *)calloc( i_text_max, sizeof( *pi_k_durations ) );			// sunqueen modify
                }
...
static int Create( vlc_object_t *p_this )
{
    ...
    /* Allocate structure */
    p_filter->p_sys = p_sys = (filter_sys_t *)malloc( sizeof(*p_sys) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
