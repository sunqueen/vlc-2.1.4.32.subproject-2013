                                               ...
                                               const uint8_t *data, int size)
{
    ...
    bool *bitmap = (bool *)malloc(bitmap_size * sizeof(*bitmap));			// sunqueen modify
    if (!bitmap)
        return NULL;
...
static subpicture_t *Decode(decoder_t *dec, block_t **block)
{
    ...
    while (b->i_buffer > 3) {
        ...
        if (segmentation_overlay) {
            ...
            sys->segment_buffer = (uint8_t *)xrealloc(sys->segment_buffer,			// sunqueen modify
...
static int Open(vlc_object_t *object)
{
    ...
    decoder_sys_t *sys = dec->p_sys = (decoder_sys_t *)malloc(sizeof(*sys));			// sunqueen modify
    if (!sys)
        return VLC_ENOMEM;
    sys->segment_id = -1;
    sys->segment_size = 0;
