...
static int OpenEncoder( vlc_object_t *p_this )
{
    ...
    p_sys->p_buffer = (uint8_t *)xmalloc(p_sys->i_frame_samples			// sunqueen modify
...
static block_t *EncodeFrames( encoder_t *p_enc, block_t *p_aout_buf )
{
    ...
    for ( int i = 0; i < i_num_frames; ++i )
    {
        ...
        memcpy( frame + 6 + i_kept_bytes, p_aout_buf->p_buffer + i_bytes_consumed,
                i_consume_bytes );
#else
        swab( (char *)p_sys->p_buffer, (char *)(frame + 6), i_kept_bytes );			// sunqueen modify
        swab( (char *)(p_aout_buf->p_buffer + i_bytes_consumed), (char *)(frame + 6 + i_kept_bytes),			// sunqueen modify
              i_consume_bytes );
#endif
