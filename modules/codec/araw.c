...
{
    uint8_t *out = (uint8_t *)outp;			// sunqueen modify
...
{
    uint16_t *out = (uint16_t *)outp;			// sunqueen modify
...
{
    swab( (char *)in, (char *)out, samples * 2 );			// sunqueen modify
}

static void S20BDecode( void *outp, const uint8_t *in, unsigned samples )
{
    int32_t *out = (int32_t *)outp;			// sunqueen modify
...
{
    uint32_t *out = (uint32_t *)outp;			// sunqueen modify
...
{
    float *out = (float *)outp;			// sunqueen modify
...
{
    double *out = (double *)outp;			// sunqueen modify
...
static block_t *Encode( encoder_t *enc, block_t *in )
{
    ...
    void (*encode)(void *, const uint8_t *, unsigned) = (void (__cdecl *)(void *,const uint8_t *,unsigned int))enc->p_sys;			// sunqueen modify
    if( encode != NULL )
...
static int EncoderOpen( vlc_object_t *p_this )
{
    ...
    p_enc->p_sys = (encoder_sys_t *)encode;			// sunqueen modify
