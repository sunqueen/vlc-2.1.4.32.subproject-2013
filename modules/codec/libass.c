...
static int Create( vlc_object_t *p_this )
{
    ...
    for( int k = 0; k < i_attachments; k++ )
    {
        ...
        if( found )
        {
            ...
            ass_add_font( p_sys->p_library, p_attach->psz_name, (char *)p_attach->p_data, p_attach->i_data );			// sunqueen modify
        }
    ...
    ass_process_codec_private( p_track, (char *)p_dec->fmt_in.p_extra, p_dec->fmt_in.i_extra );			// sunqueen modify
...
static subpicture_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    *pp_block = NULL;
    ...
    subpicture_updater_sys_t *p_spu_sys = (subpicture_updater_sys_t *)malloc( sizeof(*p_spu_sys) );			// sunqueen modify
    if( !p_spu_sys )
    {
        block_Release( p_block );
        return NULL;
    ...
    if( p_sys->p_track )
    {
        ass_process_chunk( p_sys->p_track, (char *)p_spu_sys->p_subs_data, p_spu_sys->i_subs_len,			// sunqueen modify
...
static int BuildRegions( rectangle_t *p_region, int i_max_region, ASS_Image *p_img_list, int i_width, int i_height )
{
    ...
    ASS_Image **pp_img = (ASS_Image **)calloc( i_count, sizeof(*pp_img) );			// sunqueen modify
    if( !pp_img )
        return 0;
