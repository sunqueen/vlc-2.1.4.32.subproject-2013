...
static int OpenDecoder( vlc_object_t *p_this )
{
    ...
    p_dec->p_sys = p_sys = (decoder_sys_t *)calloc( 1, sizeof( *p_sys ) );			// sunqueen modify
    if( p_sys == NULL )
        return VLC_ENOMEM;
...
static subpicture_t *ParseText( decoder_t *p_dec, block_t *p_block )
{
    ...
    psz_subtitle = (char *)malloc( p_block->i_buffer + 1 );			// sunqueen modify
    ...
    else
    {
        ...
        if( !p_sys->b_autodetect_utf8 )
        {
            ...
            char *psz_new_subtitle = (char *)xmalloc( outbytes_left + 1 );			// sunqueen modify
            ...
            *psz_convert_buffer_out++ = '\0';
            ...
            psz_subtitle = (char *)realloc( psz_new_subtitle,			// sunqueen modify
...
static char *StripTags( char *psz_subtitle )
{
    ...
    psz_text = psz_text_start = (char *)malloc( strlen( psz_subtitle ) + 1 );			// sunqueen modify
    if( !psz_text_start )
        return NULL;
    ...
    *psz_text = '\0';
    char *psz = (char *)realloc( psz_text_start, strlen( psz_text_start ) + 1 );			// sunqueen modify
...
static void HtmlNPut( char **ppsz_html, const char *psz_text, int i_max )
{
    ...
    psz_html = (char *)realloc( psz_html, i_offset + i_len + 1 );			// sunqueen modify
    if( psz_html != NULL )
    {
...
{
    char *psz_tag = (char *)malloc( 1 );			// sunqueen modify
    if( psz_tag == NULL )
        return NULL;

    char *psz_html = (char *)malloc( 1 );			// sunqueen modify
    if( psz_html == NULL )
    {
        free( psz_tag );
        return NULL;
