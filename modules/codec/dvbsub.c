...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys = p_dec->p_sys = (decoder_sys_t *)calloc( 1, sizeof(decoder_sys_t) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
#endif
        p_clut = (dvbsub_clut_t *)malloc( sizeof( dvbsub_clut_t ) );			// sunqueen modify
        if( !p_clut )
            return;
...
static void decode_page_composition( decoder_t *p_dec, bs_t *s )
{
...
#endif
        /* Allocate a new page */
        p_sys->p_page = (dvbsub_page_t *)malloc( sizeof(dvbsub_page_t) );			// sunqueen modify
        if( !p_sys->p_page )
            return;
    }
    ...
    p_sys->p_page->p_region_defs = (dvbsub_regiondef_t *)			// sunqueen modify
...
static void decode_region_composition( decoder_t *p_dec, bs_t *s )
{
...
#endif
        p_region = *pp_region = (dvbsub_region_t *)calloc( 1, sizeof(dvbsub_region_t) );			// sunqueen modify
        if( !p_region )
            return;
        ...
        ( p_region->i_height != i_height ) )
    {
        ...
        p_region->p_pixbuf = (uint8_t *)xmalloc( i_height * i_width );			// sunqueen modify
        p_region->i_depth = 0;
        b_fill = true;
    }
    if( p_region->i_depth &&
    ...
    while( i_processed_length < i_segment_length )
    {
        ...
        p_region->p_object_defs = (dvbsub_objectdef_t *)xrealloc( p_region->p_object_defs,			// sunqueen modify
    ...
    else
    {
             ...
             p_region = p_region->p_next )
        {
            for( i = 0; i < p_region->i_object_defs; i++ )
            {
                ...
                p_region->p_object_defs[i].psz_text = (char *)			// sunqueen modify
...
static subpicture_t *YuvaYuvp( subpicture_t *p_subpic )
{
    ...
    for( p_region = p_subpic->p_region; p_region; p_region = p_region->p_next )
    {
        ...
        pi_delta = (int *)xmalloc( ( p_region->p_picture->p[0].i_pitch + 1 )			// sunqueen modify
                            * sizeof(int) * 4  );
                                     ...
                                     subpicture_t *p_subpic )
{
         ...
         p_region; p_region = p_region->p_next, i_regions++ )
    {
        if( i_regions >= p_sys->i_regions )
        {
            ...
            p_sys->p_regions = (encoder_region_t *)xrealloc( p_sys->p_regions,			// sunqueen modify
