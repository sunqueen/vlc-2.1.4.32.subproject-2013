                                 ...
                                 const char *psz_tag_start )
{
    ...
    {
        char *psz_tag_end = (char *)strchr( psz_tag_start, '>' );			// sunqueen modify
...
static int ParseImageAttachments( decoder_t *p_dec )
{
    ...
    for( k = 0; k < i_attachments_cnt; k++ )
    {
            ...
            ( p_attach->p_data != NULL ) )
        {
            ...
            if( p_pic )
            {
                image_attach_t *p_picture = (image_attach_t *)malloc( sizeof(image_attach_t) );			// sunqueen modify
        ...
        switch( type )
        {
            ...
            case XML_READER_STARTELEM:
                ...
                else if( !strcasecmp( "style", node ) && (i_style_level == 1) )
                {
                    ...
                    p_ssa_style = (ssa_style_t *)calloc( 1, sizeof(ssa_style_t) );			// sunqueen modify
...
static void ParseUSFHeader( decoder_t *p_dec )
{
    ...
    p_sub = stream_MemoryNew( VLC_OBJECT(p_dec),
                              (uint8_t *)p_dec->fmt_in.p_extra,			// sunqueen modify
