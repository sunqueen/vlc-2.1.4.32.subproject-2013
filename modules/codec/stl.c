...
static char *ParseText(uint8_t *data, int size, const char *charset)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        text = (char *)realloc_or_free(text, t_size + text_size + 1);			// sunqueen modify
        if (!text)
            continue;
...
static subpicture_t *Decode(decoder_t *dec, block_t **block)
{
    ...
    uint8_t *payload = (uint8_t *)malloc(payload_size);			// sunqueen modify
    if (!payload)
        goto exit;
...
{
    uint8_t *header = (uint8_t *)dec->fmt_in.p_extra;			// sunqueen modify
    if (!header) {
    ...
    *cct_number = (cct_number_value_t)cct;			// sunqueen modify
...
static int Open(vlc_object_t *object)
{
    ...
    decoder_sys_t *sys = (decoder_sys_t *)malloc(sizeof(*sys));			// sunqueen modify
    if (!sys)
        return VLC_ENOMEM;
