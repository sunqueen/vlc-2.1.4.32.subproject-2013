...
static int OpenDecoder( vlc_object_t *p_this )
{
    ...
    p_sys = (decoder_sys_t *)malloc(sizeof(decoder_sys_t));			// sunqueen modify
    if( p_sys == NULL )
        return VLC_ENOMEM;
...
{
    struct picture_free_t *p_free = (picture_free_t *)priv;			// sunqueen modify
...
static SchroFrame *CreateSchroFrameFromPic( decoder_t *p_dec )
{
    ...
    p_free = (picture_free_t *)malloc( sizeof( *p_free ) );			// sunqueen modify
    p_free->p_pic = p_pic;
    p_free->p_dec = p_dec;
...
{
    block_t *p_block = (block_t *)priv;			// sunqueen modify
...
static picture_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    if ( *pp_block ) {
        ...
        if( p_block->i_pts > VLC_TS_INVALID ) {
            mtime_t *p_pts = (mtime_t *)malloc( sizeof(*p_pts) );			// sunqueen modify
            if( p_pts ) {
                *p_pts = p_block->i_pts;
...
static struct
{
    ...
    {0, 0, (SchroVideoFormatEnum)0},			// sunqueen modify
};
...
static int OpenEncoder( vlc_object_t *p_this )
{
    ...
    /* Allocate the memory needed to store the decoder's structure */
    if( ( p_sys = (encoder_sys_t *)calloc( 1, sizeof( *p_sys ) ) ) == NULL )			// sunqueen modify
        return VLC_ENOMEM;
...
{
    struct enc_picture_free_t *p_free = (enc_picture_free_t *)priv;			// sunqueen modify
...
static SchroFrame *CreateSchroFrameFromInputPic( encoder_t *p_enc,  picture_t *p_pic )
{
    ...
    p_free = (enc_picture_free_t *)malloc( sizeof( *p_free ) );			// sunqueen modify
