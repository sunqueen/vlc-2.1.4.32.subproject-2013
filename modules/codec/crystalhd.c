...
/* Not ready for production yet
    ...
    p_sys = (decoder_sys_t *)malloc( sizeof(*p_sys) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
#define LOAD_SYM( cast, a ) \
    BC_FUNC( a )  = cast GetProcAddress( p_sys->p_bcm_dll, ( #a ) ); \
    if( !BC_FUNC( a ) ) { \
        msg_Err( p_dec, "missing symbol " # a ); return VLC_EGENERIC; }

#define LOAD_SYM_PSYS( cast, a ) \
    p_sys->BC_FUNC( a )  = cast GetProcAddress( p_sys->p_bcm_dll, #a ); \
    if( !p_sys->BC_FUNC( a ) ) { \
        msg_Err( p_dec, "missing symbol " # a ); return VLC_EGENERIC; }

    BC_STATUS (WINAPI *OurDtsDeviceOpen)( HANDLE *hDevice, U32 mode );
    LOAD_SYM( (BC_STATUS (__stdcall *)(HANDLE *,U32)), DtsDeviceOpen );
    BC_STATUS (WINAPI *OurDtsCrystalHDVersion)( HANDLE  hDevice, PBC_INFO_CRYSTAL bCrystalInfo );
    LOAD_SYM( (BC_STATUS (__stdcall *)(HANDLE,PBC_INFO_CRYSTAL)), DtsCrystalHDVersion );
    BC_STATUS (WINAPI *OurDtsSetColorSpace)( HANDLE hDevice, BC_OUTPUT_FORMAT Mode422 );
    LOAD_SYM( (BC_STATUS (__stdcall *)(HANDLE,BC_OUTPUT_FORMAT)), DtsSetColorSpace );
    BC_STATUS (WINAPI *OurDtsSetInputFormat)( HANDLE hDevice, BC_INPUT_FORMAT *pInputFormat );
    LOAD_SYM( (BC_STATUS (__stdcall *)(HANDLE,BC_INPUT_FORMAT *)), DtsSetInputFormat );
    BC_STATUS (WINAPI *OurDtsOpenDecoder)( HANDLE hDevice, U32 StreamType );
    LOAD_SYM( (BC_STATUS (__stdcall *)(HANDLE,U32)), DtsOpenDecoder );
    BC_STATUS (WINAPI *OurDtsStartDecoder)( HANDLE hDevice );
    LOAD_SYM( (BC_STATUS (__stdcall *)(HANDLE)), DtsStartDecoder );
    BC_STATUS (WINAPI *OurDtsStartCapture)( HANDLE hDevice );
    LOAD_SYM( (BC_STATUS (__stdcall *)(HANDLE)), DtsStartCapture );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE)), DtsCloseDecoder );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE)), DtsDeviceClose );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE,U32)), DtsFlushInput );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE)), DtsStopDecoder );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE,BC_DTS_STATUS *)), DtsGetDriverStatus );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE,U8 *,U32,U64,BOOL)), DtsProcInput );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE,U32,BC_DTS_PROC_OUT *)), DtsProcOutput );
    LOAD_SYM_PSYS( (BC_STATUS (__stdcall *)(HANDLE,U8 *)), DtsIsEndOfStream );
#undef LOAD_SYM
#undef LOAD_SYM_PSYS
    ...
    p_in.mSubtype    = (BC_MEDIA_SUBTYPE)i_bcm_codec_subtype;			// sunqueen modify
...
static picture_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    proc_out.AppCallBack    = (dts_pout_callback)ourCallback;			// sunqueen modify
    proc_out.hnd            = p_dec;
                                   ...
                                   uint32_t i_buf_size)
{
    ...
    p_sys->p_sps_pps_buf = (uint8_t *)malloc( p_dec->fmt_in.i_extra * 2 );			// sunqueen modify
