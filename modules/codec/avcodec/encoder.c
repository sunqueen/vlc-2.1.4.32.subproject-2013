 ...
 * $Id: 02a7acd73405e856d8ff3baa34bc777a35252eb3 $
 *
...
int OpenEncoder( vlc_object_t *p_this )
{
    ...
    free( psz_encoder );
    if( !p_codec )
        p_codec = avcodec_find_encoder( (AVCodecID)i_codec_id );			// sunqueen modify
    if( !p_codec )
    {
...
"Do not contact the VideoLAN project about this issue.\n"),
    ...
    if( ( p_sys = (encoder_sys_t *)calloc( 1, sizeof(encoder_sys_t) ) ) == NULL )			// sunqueen modify
        return VLC_ENOMEM;
    p_enc->p_sys = p_sys;
...
/* These require libavcodec with libfdk-aac */
    ...
    if( p_enc->fmt_in.i_cat == VIDEO_ES )
    {
        ...
        GetFfmpegChroma( (int *)&p_context->pix_fmt, &p_enc->fmt_in.video );			// sunqueen modify
        ...
        {
            p_context->intra_matrix = (uint16_t *)mpeg4_default_intra_matrix;			// sunqueen modify
            p_context->inter_matrix = (uint16_t *)mpeg4_default_non_intra_matrix;		// sunqueen modify
        }
    ...
    if( i_codec_id == AV_CODEC_ID_FLAC )
    {
        ...
        {
            uint8_t *p = (uint8_t *)p_enc->fmt_out.p_extra;			// sunqueen modify
            p[0] = 0x66;    /* f */
            p[1] = 0x4C;    /* L */
            p[2] = 0x61;    /* a */
            p[3] = 0x43;    /* C */
    ...
    if( p_enc->fmt_in.i_cat == AUDIO_ES )
    {
        ...
        p_sys->p_buffer = (uint8_t *)malloc( p_sys->i_buffer_out );			// sunqueen modify
        ...
        if( p_sys->b_planar )
        {
            p_sys->p_interleave_buf = (uint8_t *)malloc( p_sys->i_buffer_out );			// sunqueen modify
...
static block_t *EncodeVideo( encoder_t *p_enc, picture_t *p_pict )
{
    ...
    if( likely(p_pict) ) {
        ...
        frame->pict_type = (AVPictureType)0;			// sunqueen modify
