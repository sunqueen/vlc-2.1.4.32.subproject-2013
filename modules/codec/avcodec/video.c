                      ...
                      AVCodec *p_codec, int i_codec_id, const char *psz_namecodec )
{
    ...
    if( ( p_dec->p_sys = p_sys = (decoder_sys_t *)calloc( 1, sizeof(decoder_sys_t) ) ) == NULL )			// sunqueen modify
        return VLC_ENOMEM;

    p_codec->type = AVMEDIA_TYPE_VIDEO;
    p_context->codec_type = AVMEDIA_TYPE_VIDEO;
    p_context->codec_id = (AVCodecID)i_codec_id;			// sunqueen modify
...
# endif
    ...
    if( p_dec->fmt_in.video.p_palette )
    {
        ...
        p_dec->fmt_out.video.p_palette = (video_palette_t *)malloc( sizeof(video_palette_t) );			// sunqueen modify
...
static void ffmpeg_InitCodec( decoder_t *p_dec )
{
    ...
    if( p_sys->i_codec_id == AV_CODEC_ID_SVQ3 )
    {
        ...
        p = p_sys->p_context->extradata = (uint8_t *)			// sunqueen modify
    ...
    else
    {
        ...
        p_sys->p_context->extradata = (uint8_t *)			// sunqueen modify
...
static int ffmpeg_va_Start( void *func, va_list ap )
{
    ...
    int (*open)( vlc_va_t *, int, const es_format_t * ) = (int (__cdecl *)(vlc_va_t *,int,const es_format_t *))func;			// sunqueen modify
...
{
    vlc_va_t *p_va = (vlc_va_t *)vlc_object_create( parent, sizeof( *p_va ) );			// sunqueen modify
...
{
    vlc_va_t *va = va_arg( ap, vlc_va_t * );
    void (*close)( vlc_va_t * ) = (void (__cdecl *)(vlc_va_t *))func;			// sunqueen modify
...
{
    decoder_t *p_dec = (decoder_t *)p_context->opaque;			// sunqueen modify
    decoder_sys_t *p_sys = p_dec->p_sys;
    vlc_va_t *p_va = p_sys->p_va;
