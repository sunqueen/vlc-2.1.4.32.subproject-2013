...
static void InitDecoderConfig( decoder_t *p_dec, AVCodecContext *p_context )
{
    ...
    {
        const uint8_t * const p_src = (const uint8_t *const )p_dec->fmt_in.p_extra;			// sunqueen modify
        ...
        if( i_size > 0 )
        {
            p_context->extradata = (uint8_t *)			// sunqueen modify
...
static int GetAudioBuf( AVCodecContext *ctx, AVFrame *buf )
{
    ...
    {
        uint8_t **ext = (uint8_t **)malloc( sizeof( *ext ) * planes );			// sunqueen modify
...
block_t * DecodeAudio ( decoder_t *p_dec, block_t **pp_block )
{
    ...
    p_block = (block_t *)frame.opaque;			// sunqueen modify
