 ...
 * $Id: 0953c4f2be9bdc5d5e5fa3f9c88c0adf84484f88 $
 *
...
static int OpenDecoder( vlc_object_t *p_this )
{
    ...
    decoder_t **list = (decoder_t **)realloc( kate_decoder_list, (kate_decoder_list_size+1) * sizeof( *list ));			// sunqueen modify
    if( list )
    {
...
static subpicture_t *DecodePacket( decoder_t *p_dec, kate_packet *p_kp, block_t *p_block )
{
    ...
    if( p_sys->b_use_tiger)
    {
        p_spu_sys = (subpicture_updater_sys_t *)malloc( sizeof(*p_spu_sys) );			// sunqueen modify
        if( !p_spu_sys )
            return NULL;
    }
...
// sunqueen add end
#endif
        /*.p_sys       =*/ p_spu_sys,			// sunqueen modify
    };
