...
found:
    p_sys = p_dec->p_sys = (decoder_sys_t *)malloc(sizeof(*p_sys));			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
static int DecOpen( decoder_t *p_dec )
{
    ...
    if( p_dec->fmt_in.i_cat == AUDIO_ES )
    {
        ...
        p_wf = (WAVEFORMATEX *)malloc( i_size );			// sunqueen modify
    ...
    else
    {
        ...
        p_vih = (VIDEOINFOHEADER *)malloc( i_size );			// sunqueen modify
    ...
    /* Find out some properties of the output */
    {
        ...
        else
        {
            ...
            p_sys->p_buffer = (uint8_t *)malloc( i_size );			// sunqueen modify
                    ...
                    bool b_out )
{
    ...
    *p_hmsdmo_dll = LoadLibraryA( "msdmo.dll" );
    ...
    OurDMOEnum = (long (__stdcall *)(const GUID *,uint32_t,uint32_t,const DMO_PARTIAL_MEDIATYPE *,uint32_t,const DMO_PARTIAL_MEDIATYPE *,IEnumDMO **))GetProcAddress( *p_hmsdmo_dll, "DMOEnum" );			// sunqueen modify
    if( OurDMOEnum == NULL )
    {
    ...
    *pp_dmo = 0;
                     ...
                     &psz_dmo_name, &i_dummy /* NULL doesn't work */ ) ) )
    {
        ...
        if( CoCreateInstance( clsid_dmo, NULL, CLSCTX_INPROC,
                              IID_IMediaObject, (void **)pp_dmo ) )			// sunqueen modify
        {
...
loader:
    ...
    *p_hmsdmo_dll = LoadLibraryA( codecs_table[i_codec].psz_dll );
    ...
    i_err = cFactory->vt->CreateInstance( (IClassFactoryHack *)cFactory, (IUnknownHack *)0, (const GUID *)&IID_IUnknown,
                                          (void **)&cObject );			// sunqueen modify
    ...
    i_err = cObject->vt->QueryInterface( (IUnknownHack *)cObject, &IID_IMediaObject,
                                        (void **)pp_dmo );			// sunqueen modify
...
{
    decoder_t *p_dec = (decoder_t *)data;			// sunqueen modify
    decoder_sys_t *p_sys = p_dec->p_sys;
...
static int EncoderSetVideoType( encoder_t *p_enc, IMediaObject *p_dmo )
{
    ...
    while( 1 )
    {
        ...
        p_data = (uint8_t *)malloc( i_data );			// sunqueen modify
        ...
        p_vih = (VIDEOINFOHEADER *)CoTaskMemAlloc( i_vih );			// sunqueen modify
...
static int EncOpen( vlc_object_t *p_this )
{
    ...
    /* Allocate the memory needed to store the decoder's structure */
    if( ( p_enc->p_sys = p_sys = (encoder_sys_t *)malloc(sizeof(*p_sys)) ) == NULL )			// sunqueen modify
    {
        goto error;
    }
