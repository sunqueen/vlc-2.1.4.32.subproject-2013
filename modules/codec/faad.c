...
static int Open( vlc_object_t *p_this )
{
    ...
    /* Open a faad context */
    if( ( p_sys->hfaad = (NeAACDecHandle *)faacDecOpen() ) == NULL )			// sunqueen modify
    {
    ...
    if( p_dec->fmt_in.i_extra > 0 )
    {
        ...
        if( faacDecInit2( p_sys->hfaad, (unsigned char *)p_dec->fmt_in.p_extra,			// sunqueen modify
...
static block_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    if( p_sys->i_buffer_size < p_sys->i_buffer + p_block->i_buffer )
    {
        ...
        uint8_t *p_buffer     = (uint8_t *)realloc( p_sys->p_buffer, i_buffer_size );			// sunqueen modify
        if( p_buffer )
        {
    ...
    if( p_sys->i_buffer )
    {
        ...
        if( frame.error > 0 )
        {
            ...
            if( frame.error == 21 || frame.error == 12 )
            {
                ...
                hfaad = (NeAACDecHandle *)faacDecOpen();			// sunqueen modify
        ...
        DoReordering( (uint32_t *)p_out->p_buffer, (uint32_t *)samples,			// sunqueen modify
