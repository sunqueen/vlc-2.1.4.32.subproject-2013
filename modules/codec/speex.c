 ...
 * $Id: 94fc27e1e4131ff5dd2c91866594b0e7971470fd $
 *
...
static block_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    return (block_t *)ProcessPacket( p_dec, &oggpacket, pp_block );			// sunqueen modify
}
...
static int OpenEncoder( vlc_object_t *p_this )
{
    ...
    p_sys->p_buffer = (char *)xmalloc( p_sys->i_frame_size );			// sunqueen modify
    ...
    p_extra = (uint8_t *)(p_enc->fmt_out.p_extra = xmalloc( p_enc->fmt_out.i_extra ));			// sunqueen modify
    for( i = 0; i < 2; i++ )
    {
