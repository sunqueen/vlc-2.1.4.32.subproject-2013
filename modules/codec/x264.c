...
static int  Open ( vlc_object_t *p_this )
{
    ...
    p_enc->p_sys = p_sys = (encoder_sys_t *)malloc( sizeof( encoder_sys_t ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
#   define X264_ANALYSE_BSUB16x16 0
    ...
    uint8_t *p_extra = (uint8_t *)(p_enc->fmt_out.p_extra = malloc( i_extra ));			// sunqueen modify
    if( !p_extra )
    {
        Close( VLC_OBJECT(p_enc) );
        return VLC_ENOMEM;
    ...
    for( i = 0; i < i_nal; i++ )
    {
        ...
        p_sys->p_sei = (uint8_t *)malloc( p_sys->i_sei_size );			// sunqueen modify
        if( !p_sys->p_sei )
        {
            free( p_extra );
            Close( VLC_OBJECT(p_enc) );
