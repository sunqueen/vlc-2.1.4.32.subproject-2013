...
static int OpenDecoder( vlc_object_t *p_this )
{
    ...
    if( ( p_dec->p_sys = (decoder_sys_t *)malloc(sizeof(decoder_sys_t)) ) == NULL )			// sunqueen modify
        return VLC_ENOMEM;
...
static picture_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    /* Decode picture */
    p_row_pointers = (png_bytep *)malloc( sizeof(png_bytep) * i_height );			// sunqueen modify
    if( !p_row_pointers )
        goto error;
