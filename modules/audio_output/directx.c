...
static int InitDirectSound( audio_output_t *p_aout )
{
    ...
    OurDirectSoundCreate = (HRESULT (__stdcall *)(LPGUID,LPDIRECTSOUND *,LPUNKNOWN))			// sunqueen modify
                           ...
                           bool b_probe )
{
    ...
    if( IDirectSoundBuffer_QueryInterface( p_aout->sys->p_dsbuffer,
                                           IID_IDirectSoundNotify,			// sunqueen modify
...
{
    ds_list_t *list = (ds_list_t *)data;			// sunqueen modify
    OLECHAR buf[48];

    if( StringFromGUID2( *guid, buf, 48 ) <= 0 )			// sunqueen modify
        return true;

    list->count++;
    list->ids = (char **)xrealloc( list->ids, list->count * sizeof(char *) );			// sunqueen modify
    list->names = (char **)xrealloc( list->names, list->count * sizeof(char *) );			// sunqueen modify
                                 ...
                                 char ***values, char ***descs )
{
    ...
    HRESULT (WINAPI *OurDirectSoundEnumerate)(LPDSENUMCALLBACKW, LPVOID) =
            (HRESULT (__stdcall *)(LPDSENUMCALLBACKW,LPVOID))GetProcAddress( (HMODULE)hdsound_dll, "DirectSoundEnumerateW" );			// sunqueen modify
    if( OurDirectSoundEnumerate != NULL )
    {
        OurDirectSoundEnumerate( DeviceEnumCallback, &list );
        msg_Dbg( p_this, "found %u devices", list.count );
    }
    FreeLibrary((HMODULE)hdsound_dll);			// sunqueen modify
...
static int Open(vlc_object_t *obj)
{
    ...
    aout_sys_t *sys = (aout_sys_t *)calloc(1, sizeof (*sys));			// sunqueen modify
