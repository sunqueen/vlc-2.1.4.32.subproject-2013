...
static inline aout_sys_t *vlc_AudioSessionEvents_sys(IAudioSessionEvents *_this)			// sunqueen modify
{
	return (aout_sys_t *)(((char *)_this) - offsetof(aout_sys_t, session_events));			// sunqueen modify
}

static STDMETHODIMP
vlc_AudioSessionEvents_QueryInterface(IAudioSessionEvents *_this, REFIID riid,			// sunqueen modify
                                      void **ppv)
{
	if (IsEqualIID(riid, IID_IUnknown)			// sunqueen modify
     || IsEqualIID(riid, IID_IAudioSessionEvents))			// sunqueen modify
    {
        *ppv = _this;
		IUnknown_AddRef(_this);			// sunqueen modify
        return S_OK;
    }
    else
    {
...
vlc_AudioSessionEvents_AddRef(IAudioSessionEvents *_this)			// sunqueen modify
{
	aout_sys_t *sys = vlc_AudioSessionEvents_sys(_this);			// sunqueen modify
    return InterlockedIncrement(&sys->refs);
}

static STDMETHODIMP_(ULONG)
vlc_AudioSessionEvents_Release(IAudioSessionEvents *_this)			// sunqueen modify
{
	aout_sys_t *sys = vlc_AudioSessionEvents_sys(_this);			// sunqueen modify
...
static STDMETHODIMP
vlc_AudioSessionEvents_OnDisplayNameChanged(IAudioSessionEvents *_this,			// sunqueen modify
...
static STDMETHODIMP
vlc_AudioSessionEvents_OnIconPathChanged(IAudioSessionEvents *_this,			// sunqueen modify
...
static STDMETHODIMP
vlc_AudioSessionEvents_OnSimpleVolumeChanged(IAudioSessionEvents *_this,			// sunqueen modify
...
static STDMETHODIMP
vlc_AudioSessionEvents_OnChannelVolumeChanged(IAudioSessionEvents *_this,			// sunqueen modify
...
static STDMETHODIMP
vlc_AudioSessionEvents_OnGroupingParamChanged(IAudioSessionEvents *_this,			// sunqueen modify
...
static STDMETHODIMP
vlc_AudioSessionEvents_OnStateChanged(IAudioSessionEvents *_this,			// sunqueen modify
...
static STDMETHODIMP
vlc_AudioSessionEvents_OnSessionDisconnected(IAudioSessionEvents *_this,			// sunqueen modify
...
{
	audio_output_t *aout = (audio_output_t *)data;			// sunqueen modify
    aout_sys_t *sys = aout->sys;
...
static int DevicesEnum(audio_output_t *aout)
{
    ...
    for (UINT i = 0; i < count; i++)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            PropVariantInit(&v);
			hr = IPropertyStore_GetValue(props, PKEY_Device_FriendlyName, &v);			// sunqueen modify
            if (SUCCEEDED(hr))
...
static HRESULT OpenDevice(audio_output_t *aout, const char *devid)
{
    ...
    void *pv;
	hr = IMMDevice_Activate(sys->dev, IID_IAudioSessionManager,			// sunqueen modify
                            CLSCTX_ALL, NULL, &pv);
    if (FAILED(hr))
        msg_Err(aout, "cannot activate session manager (error 0x%lx)", hr);
    else
		sys->manager = (IAudioSessionManager *)pv;			// sunqueen modify
...
static HRESULT ActivateDevice(void *opaque, REFIID iid, PROPVARIANT *actparms,
                              void **restrict pv)
{
	IMMDevice *dev = (IMMDevice *)opaque;			// sunqueen modify
...
static int Start(audio_output_t *aout, audio_sample_format_t *restrict fmt)
{
	...
	aout_stream_t *s = (aout_stream_t *)vlc_object_create(aout, sizeof (*s));			// sunqueen modify
    if (unlikely(s == NULL))
        return -1;
...
static int Open(vlc_object_t *obj)
{
	...
	aout_sys_t *sys = (aout_sys_t *)malloc(sizeof (*sys));			// sunqueen modify
    ...
    hr = CoCreateInstance(CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL,
                          IID_IMMDeviceEnumerator, &pv);			// sunqueen modify
    if (FAILED(hr))
    {
        LeaveMTA();
	...
	sys->it = (IMMDeviceEnumerator *)pv;			// sunqueen modify
