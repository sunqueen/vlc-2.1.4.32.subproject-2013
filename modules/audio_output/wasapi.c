...
{
	aout_stream_sys_t *sys = (aout_stream_sys_t *)s->sys;			// sunqueen modify
    void *pv;
    UINT64 pos, qpcpos;
    HRESULT hr;

	hr = IAudioClient_GetService(sys->client, IID_IAudioClock, &pv);			// sunqueen modify
    if (SUCCEEDED(hr))
    {
		IAudioClock *clock = (IAudioClock *)pv;			// sunqueen modify
...
static HRESULT Play(aout_stream_t *s, block_t *block)
{
	...
	hr = IAudioClient_GetService(sys->client, IID_IAudioRenderClient, &pv);			// sunqueen modify
    if (FAILED(hr))
    {
	...
	IAudioRenderClient *render = (IAudioRenderClient *)pv;			// sunqueen modify
    for (;;)
    {
        UINT32 frames;
                        ...
                        audio_sample_format_t *restrict audio)
{
    ...
    {
		const WAVEFORMATEXTENSIBLE *wfe = (WAVEFORMATEXTENSIBLE *)wf;			// sunqueen modify
...
static HRESULT Start(aout_stream_t *s, audio_sample_format_t *restrict fmt,
                     const GUID *sid)
{
	aout_stream_sys_t *sys = (aout_stream_sys_t *)malloc(sizeof (*sys));			// sunqueen modify
    if (unlikely(sys == NULL))
        return E_OUTOFMEMORY;
    sys->client = NULL;

    void *pv;
	HRESULT hr = aout_stream_Activate(s, IID_IAudioClient, NULL, &pv);			// sunqueen modify
    if (FAILED(hr))
    {
        msg_Err(s, "cannot activate client (error 0x%lx)", hr);
        goto error;
    }
	sys->client = (IAudioClient *)pv;			// sunqueen modify
