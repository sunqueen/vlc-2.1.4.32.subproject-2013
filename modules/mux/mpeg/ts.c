...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys = (sout_mux_sys_t *)calloc( 1, sizeof( sout_mux_sys_t ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
    p_sys->i_num_pmt = 1;
...
static int AddStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
    ...
    p_input->p_sys = p_stream = (ts_stream_t *)calloc( 1, sizeof( ts_stream_t ) );			// sunqueen modify
    if( !p_stream )
        goto oom;
    ...
    p_stream->lang = (uint8_t *)calloc(1, p_stream->i_langs * 4);			// sunqueen modify
    if( !p_stream->lang )
        goto oom;
    ...
    {
        p_stream->i_extra = 55;
        p_stream->p_extra = (uint8_t *)malloc( p_stream->i_extra );			// sunqueen modify
        if (!p_stream->p_extra)
            goto oom;
    ...
    else
    {
        ...
        if( fmt->i_extra > 0 )
        {
            ...
            p_stream->p_extra = (uint8_t *)malloc( fmt->i_extra );			// sunqueen modify
            if( !p_stream->p_extra )
                goto oom;
...
#define ADTS_HEADER_SIZE 7 /* CRC needs 2 more bytes */
    ...
    uint8_t *p_extra = (uint8_t *)p_fmt->p_extra;			// sunqueen modify
...
static void GetPMT( sout_mux_t *p_mux, sout_buffer_chain_t *c )
{
    ...
    {
        p_sys->dvbpmt = (dvbpsi_pmt_t *)malloc( p_sys->i_num_pmt * sizeof(dvbpsi_pmt_t) );			// sunqueen modify
        if( p_sys->dvbpmt == NULL )
            return;
    }
...
//        uint8_t psz_sdt_desc[3 + provlen + servlen];
    ...
    for (int i_stream = 0; i_stream < p_mux->i_nb_inputs; i_stream++ )
    {
        ...
        pmt_map_t *p_usepid = (pmt_map_t *)bsearch( &i_pidinput, p_sys->pmtmap,			// sunqueen modify
