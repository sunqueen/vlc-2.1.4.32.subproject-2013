...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys                 = (sout_mux_sys_t *)malloc( sizeof( sout_mux_sys_t ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
    p_sys->i_streams      = 0;
...
static int AddStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
    ...
    p_input->p_sys = p_stream = (ogg_stream_t *)calloc( 1, sizeof( ogg_stream_t ) );			// sunqueen modify
    if( !p_stream )
        return VLC_ENOMEM;
    ...
    case VIDEO_ES:
        ...
        case VLC_CODEC_WMV2:
        case VLC_CODEC_WMV3:
            p_stream->p_oggds_header = (oggds_header_t *)calloc( 1, sizeof(oggds_header_t) );			// sunqueen modify
    ...
    case AUDIO_ES:
        ...
        default:
            ...
            p_stream->p_oggds_header = (oggds_header_t *)			// sunqueen modify
...
static int DelStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
    ...
    if( p_input->p_sys )
    {
        ...
        if( !p_stream->b_new )
        {
            p_sys->pp_del_streams = (ogg_stream_t **)xrealloc( p_sys->pp_del_streams,			// sunqueen modify
...
static block_t *OggCreateHeader( sout_mux_t *p_mux )
{
    ...
    for( int pass = 0; pass < 2; pass++ )
    {
        for( i = 0; i < p_mux->i_nb_inputs; i++ )
        {
                ...
                p_stream->i_fourcc == VLC_CODEC_THEORA )
            {
                ...
                op.bytes  = pi_size[0];
                op.packet = (unsigned char *)pp_data[0];			// sunqueen modify
                if( pi_size[0] <= 0 )
            ...
            {
                op.packet = (unsigned char *)p_input->p_fmt->p_extra;			// sunqueen modify
            ...
            {
                /* ds header */
                op.packet = (unsigned char *)malloc( sizeof(*p_stream->p_oggds_header) + p_stream->p_oggds_header->i_size );			// sunqueen modify
                if( !op.packet )
                    return NULL;
    ...
    for( i = 0; i < p_mux->i_nb_inputs; i++ )
    {
            ...
            p_stream->i_fourcc == VLC_CODEC_THEORA )
        {
            ...
            {
                op.bytes  = pi_size[i];
                op.packet = (unsigned char *)pp_data[i];			// sunqueen modify
                if( pi_size[i] <= 0 )
                    msg_Err( p_mux, "header data corrupted");
...
static block_t *OggCreateFooter( sout_mux_t *p_mux )
{
    ...
    for( i = 0; i < p_mux->i_nb_inputs; i++ )
    {
        ogg_stream_t *p_stream = (ogg_stream_t *)p_mux->pp_inputs[i]->p_sys;			// sunqueen modify
