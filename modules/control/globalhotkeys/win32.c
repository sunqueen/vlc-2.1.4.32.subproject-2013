...
static int Open( vlc_object_t *p_this )
{
    ...
    intf_sys_t *p_sys = (intf_sys_t *)malloc( sizeof (intf_sys_t) );			// sunqueen modify
...
static void *Thread( void *p_data )
{
    ...
    vlc_mutex_lock( &p_sys->lock );
    p_sys->hotkeyWindow =
        (HWND)CreateWindow( _T("STATIC"),            /* name of window class */			// sunqueen modify
    ...
    if( p_sys->hotkeyWindow == NULL )
    {
        p_sys->hotkeyWindow = (HWND)INVALID_HANDLE_VALUE;			// sunqueen modify
