...
static int Open(vlc_object_t *object)
{
    ...
    intf->p_sys = sys = (intf_sys_t *)malloc(sizeof(*sys));			// sunqueen modify
    if (!sys) {
        net_Close(fd);
        return VLC_ENOMEM;
    }
...
{
    intf_thread_t *intf = (intf_thread_t *)handle;			// sunqueen modify
    intf_sys_t *sys = intf->p_sys;
    for (;;) {
        struct pollfd ufd = { /*.fd =*/ sys->fd, /*.events =*/ POLLIN, };			// sunqueen modify
        uint64_t data[2];
        ...
        recvfrom(sys->fd, (char *)data, sizeof(data), 0,
                 (struct sockaddr*)&from, (int *)&struct_size);			// sunqueen modify
        ...
        /* Reply to the sender */
        sendto(sys->fd, (char *)data, sizeof(data), 0,			// sunqueen modify
...
static void *Slave(void *handle)
{
    ...
    for (;;) {
        ...
        if (send(sys->fd, (char *)data, sizeof(data[0]), 0) <= 0)			// sunqueen modify
            goto wait;
        ...
        if (recv(sys->fd, (char *)data, sizeof(data), 0) <= 0)			// sunqueen modify
            goto wait;
                      ...
                      vlc_value_t oldval, vlc_value_t newval, void *data)
{
    ...
    intf_thread_t  *intf = (intf_thread_t *)data;			// sunqueen modify
    intf_sys_t     *sys = intf->p_sys;
                         ...
                         vlc_value_t oldval, vlc_value_t newval, void *data)
{
    ...
    input_thread_t *input = (input_thread_t *)newval.p_address;			// sunqueen modify
    assert(sys->input == NULL);
    sys->input = (input_thread_t *)vlc_object_hold(input);			// sunqueen modify
