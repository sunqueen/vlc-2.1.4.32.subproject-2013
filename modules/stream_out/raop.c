                           ...
                           const unsigned char *P, const size_t pLen )
{
     ...
     */
    DB = (unsigned char *)calloc( 1, hLen + psLen + 1 + mLen );			// sunqueen modify
    dbMask = (unsigned char *)calloc( 1, emLen - hLen );			// sunqueen modify
    seedMask = (unsigned char *)calloc( 1, hLen );			// sunqueen modify
     ...
     */
    seed = (unsigned char *)gcry_random_bytes( hLen, GCRY_STRONG_RANDOM );			// sunqueen modify
    if ( seed == NULL )
    {
        i_err = VLC_ENOMEM;
        goto error;
                            ...
                            vlc_dictionary_t *p_req_headers )
{
    ...
    {
        psz_key = ppsz_keys[i];
        psz_value = (char *)vlc_dictionary_value_for_key( p_req_headers, psz_key );			// sunqueen modify
                                    ...
                                    vlc_dictionary_t *p_resp_headers )
{
    ...
    psz_auth = (char *)vlc_dictionary_value_for_key( p_resp_headers,			// sunqueen modify
                                             "WWW-Authenticate" );
    if ( psz_auth == NULL )
    {
...
static int SendSetup( vlc_object_t *p_this )
{
    ...
    psz_tmp = (char *)vlc_dictionary_value_for_key( &resp_headers, "Session" );			// sunqueen modify
    if ( !psz_tmp )
    {
    ...
    /* Get server_port */
    psz_next = (char *)vlc_dictionary_value_for_key( &resp_headers, "Transport" );			// sunqueen modify
    ...
    /* Get jack type */
    psz_next = (char *)vlc_dictionary_value_for_key( &resp_headers,			// sunqueen modify
                                             "Audio-Jack-Status" );
    while ( SplitHeader( &psz_next, &psz_name, &psz_value ) )
    {
...
static int SendRecord( vlc_object_t *p_this )
{
    ...
    psz_value = (char *)vlc_dictionary_value_for_key( &resp_headers, "Audio-Latency" );			// sunqueen modify
    if ( psz_value )
...
static void SendAudio( sout_stream_t *p_stream, block_t *p_buffer )
{
    ...
    while ( p_buffer )
    {
        ...
        if ( i_len > p_sys->i_sendbuf_len || p_sys->p_sendbuf == NULL )
        {
            ...
            p_sys->p_sendbuf = (uint8_t *)realloc_or_free( p_sys->p_sendbuf, i_realloc_len );			// sunqueen modify
...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys = (sout_stream_sys_t *)calloc( 1, sizeof( *p_sys ) );			// sunqueen modify
    if ( p_sys == NULL )
    {
        i_err = VLC_ENOMEM;
        goto error;
...
static sout_stream_id_t *Add( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    id = (sout_stream_id_t *)calloc( 1, sizeof( *id ) );			// sunqueen modify
    if ( id == NULL )
        goto error;
