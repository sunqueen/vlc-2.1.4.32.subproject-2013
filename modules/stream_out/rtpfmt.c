                                 ...
                                 uint8_t *theora_pixel_fmt)
{
    ...
    *p_buffer = (uint8_t *)malloc(*i_buffer);			// sunqueen modify
    if (*p_buffer == NULL)
    {
        val = VLC_ENOMEM;
        goto free;
                               ...
                               int64_t i_pts )
{
    ...
    char *start = (char *)strstr(fmtp, "configuration=");			// sunqueen modify
    assert(start != NULL);
...
static int rtp_packetize_swab( sout_stream_id_t *id, block_t *in )
{
    ...
    for( i = 0; i < i_count; i++ )
    {
        ...
        rtp_packetize_common( id, out, (i == i_count - 1),
                      (in->i_pts > VLC_TS_INVALID ? in->i_pts : in->i_dts) );
        swab( (char *)p_data, (char *)out->p_buffer + 12, i_payload );			// sunqueen modify
