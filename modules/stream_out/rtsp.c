...
rtsp_stream_t *RtspSetup( vlc_object_t *owner, vod_media_t *media,
                          const char *path )
{
    rtsp_stream_t *rtsp = (rtsp_stream_t *)malloc( sizeof( *rtsp ) );			// sunqueen modify
    ...
    httpd_UrlCatch( rtsp->url, HTTPD_MSG_DESCRIBE, RtspCallback, (httpd_callback_sys_t *)rtsp );			// sunqueen modify
    httpd_UrlCatch( rtsp->url, HTTPD_MSG_SETUP,    RtspCallback, (httpd_callback_sys_t *)rtsp );			// sunqueen modify
    httpd_UrlCatch( rtsp->url, HTTPD_MSG_PLAY,     RtspCallback, (httpd_callback_sys_t *)rtsp );			// sunqueen modify
    httpd_UrlCatch( rtsp->url, HTTPD_MSG_PAUSE,    RtspCallback, (httpd_callback_sys_t *)rtsp );			// sunqueen modify
    httpd_UrlCatch( rtsp->url, HTTPD_MSG_GETPARAMETER, RtspCallback,
                    (httpd_callback_sys_t *)rtsp );			// sunqueen modify
    httpd_UrlCatch( rtsp->url, HTTPD_MSG_TEARDOWN, RtspCallback, (httpd_callback_sys_t *)rtsp );			// sunqueen modify
    return rtsp;
...
static void RtspTimeOut( void *data )
{
    rtsp_stream_t *rtsp = (rtsp_stream_t *)data;			// sunqueen modify
...
{
    rtsp_session_t *s = (rtsp_session_t *)malloc( sizeof( *s ) );			// sunqueen modify
    if( s == NULL )
        return NULL;
                     ...
                     uint32_t *ssrc, uint16_t *seq_init )
{
    ...
    else
    {
        ...
        rtsp_strack_t track = { /*.id =*/ id, /*.sout_id =*/ sout_id,
                                /*.setup_fd =*/ -1, /*.rtp_fd =*/ -1 };			// sunqueen modify
        ...
        case HTTPD_MSG_SETUP:
                 ...
                 tpt = transport_next( tpt ) )
            {
                ...
                else
                {
					...
					// sunquen modify start
//                    setsockopt (fd, SOL_SOCKET, SO_RCVBUF, &(int){ 0 },
//                                sizeof (int));
					int opt = 0;
                    setsockopt (fd, SOL_SOCKET, SO_RCVBUF, (const char *)&opt/*(int){ 0 }*/,
                                sizeof (int));
					// sunqueen moidyf end
