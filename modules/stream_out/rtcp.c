                         ...
                         bool mux)
{
    ...
    else
    {
        ...
        if (fd != -1)
        {
            ...
            if (!getsockopt (rtp_fd, SOL_IP, IP_MULTICAST_TTL, (char *)&ttl, &len))			// sunqueen modify
                setsockopt (fd, SOL_IP, IP_MULTICAST_TTL, (const char *)&ttl, len);			// sunqueen modify

            /* Ignore all incoming RTCP-RR packets */
			// sunqueen modify start
//            setsockopt (fd, SOL_SOCKET, SO_RCVBUF, &(int){ 0 }, sizeof (int));
			int opt = 0;
            setsockopt (fd, SOL_SOCKET, SO_RCVBUF, (const char *)&opt/*(int){ 0 }*/, sizeof (int));
			// sunqueen modify end
        }
    }
    ...
    rtcp = (rtcp_sender_t *)malloc (sizeof (*rtcp));			// sunqueen modify
    if (rtcp == NULL)
    {
        net_Close (fd);
        return NULL;
...
void CloseRTCP (rtcp_sender_t *rtcp)
{
    ...
    send (rtcp->handle, (const char *)rtcp->payload, rtcp->length, 0);			// sunqueen modify
...
void SendRTCP (rtcp_sender_t *restrict rtcp, const block_t *rtp)
{
    ...
    if (send (rtcp->handle, (const char *)ptr, rtcp->length, 0) == (ssize_t)rtcp->length)			// sunqueen modify
        rtcp->counter = 0;
}
