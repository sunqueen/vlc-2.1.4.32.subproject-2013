...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys = (sout_stream_sys_t *)malloc( sizeof( sout_stream_sys_t ) );			// sunqueen modify
    if( p_sys == NULL )
        return VLC_ENOMEM;
    ...
    if (! strcmp(p_stream->psz_name, "vod"))
    {
         ...
         * stream output. */
        p_sys->p_vod_media = (vod_media_t *)var_InheritAddress(p_stream, "vod-media");			// sunqueen modify
...
static sout_stream_id_t *Add( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    sout_stream_id_t *id = (sout_stream_id_t *)malloc( sizeof( *id ) );			// sunqueen modify
    if( unlikely(id == NULL) )
        return NULL;
...
static int HttpSetup( sout_stream_t *p_stream, const vlc_url_t *url)
{
    ...
    if( p_sys->p_httpd_host )
    {
        p_sys->p_httpd_file = httpd_FileNew( p_sys->p_httpd_host,
                                             ...
                                             "application/sdp",
                                             NULL, NULL,
                                             HttpCallback, (httpd_file_sys_t *)p_sys );			// sunqueen modify
    }
                          ...
                          uint8_t **pp_data, int *pi_data )
{
    ...
    if( p_sys->psz_sdp && *p_sys->psz_sdp )
    {
        ...
        *pp_data = (uint8_t *)malloc( *pi_data );			// sunqueen modify
...
#endif
    sout_stream_id_t *id = (sout_stream_id_t *)data;			// sunqueen modify
...
//        int deadv[id->sinkc]; /* Dead sockets list */
        ...
        for( int i = 0; i < id->sinkc; i++ )
        {
            ...
            if( send( id->sinkv[i].rtp_fd, (const char *)out->p_buffer, len, 0 ) == -1			// sunqueen modify
             ...
             && net_errno != ENOBUFS && net_errno != ENOMEM )
            {
                ...
                if( type == SOCK_DGRAM )
                    ...
                    send( id->sinkv[i].rtp_fd, (const char *)out->p_buffer, len, 0 );			// sunqueen modify
                else
                    /* Broken connection */
...
static sout_access_out_t *GrabberCreate( sout_stream_t *p_stream )
{
    ...
    p_grab = (sout_access_out_t *)vlc_object_create( p_stream, sizeof( *p_grab ) );			// sunqueen modify
    if( p_grab == NULL )
        return NULL;
