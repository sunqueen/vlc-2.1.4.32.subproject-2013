...
static sout_stream_id_t *Add( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    id->p_decoder = (decoder_t *)vlc_object_create( p_stream, sizeof( decoder_t ) );			// sunqueen modify
    if( !id->p_decoder )
        goto error;
...
static int Del( sout_stream_t *p_stream, sout_stream_id_t *id )
{
    ...
    if( id->id ) sout_StreamIdDel( p_stream->p_next, (sout_stream_id_t *)id->id );			// sunqueen modify
                 ...
                 block_t *p_buffer )
{
    ...
    {
        if( id->id )
            return sout_StreamIdSend( p_stream->p_next, (sout_stream_id_t *)id->id, p_buffer );			// sunqueen modify
    ...
    if( p_out )
        return sout_StreamIdSend( p_stream->p_next, (sout_stream_id_t *)id->id, p_out );			// sunqueen modify
    return VLC_SUCCESS;
}
