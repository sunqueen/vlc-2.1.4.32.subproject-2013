...
static int Open( vlc_object_t * p_this )
{
    ...
    p_sys = p_demux->p_sys = (demux_sys_t *)calloc( 1, sizeof(demux_sys_t) );			// sunqueen modify
    p_sys->b_odml   = false;
    p_sys->track    = NULL;
    p_sys->meta     = NULL;
    ...
    if( AVI_ChunkCount( &p_sys->ck_root, AVIFOURCC_RIFF ) > 1 )
    {
        ...
        for( i = 1; i < i_count; i++ )
        {
            ...
            p_sysx = (avi_chunk_list_t *)AVI_ChunkFind( &p_sys->ck_root, AVIFOURCC_RIFF, i );			// sunqueen modify
    ...
    p_riff  = (avi_chunk_list_t *)AVI_ChunkFind( &p_sys->ck_root, AVIFOURCC_RIFF, 0 );			// sunqueen modify
    p_hdrl  = (avi_chunk_list_t *)AVI_ChunkFind( p_riff, AVIFOURCC_hdrl, 0 );			// sunqueen modify
    p_movi  = (avi_chunk_list_t *)AVI_ChunkFind( p_riff, AVIFOURCC_movi, 0 );			// sunqueen modify
    if( !p_movi )
        p_movi  = (avi_chunk_list_t *)AVI_ChunkFind( &p_sys->ck_root, AVIFOURCC_movi, 0 );			// sunqueen modify
    ...
    if( !( p_avih = (avi_chunk_avih_t *)AVI_ChunkFind( p_hdrl, AVIFOURCC_avih, 0 ) ) )			// sunqueen modify
    {
    ...
    for( i = 0 ; i < i_track; i++ )
    {
        avi_track_t           *tk     = (avi_track_t *)calloc( 1, sizeof( avi_track_t ) );			// sunqueen modify
        if( unlikely( !tk ) )
            goto error;

        avi_chunk_list_t      *p_strl = (avi_chunk_list_t *)AVI_ChunkFind( p_hdrl, AVIFOURCC_strl, i );			// sunqueen modify
        avi_chunk_strh_t      *p_strh = (avi_chunk_strh_t *)AVI_ChunkFind( p_strl, AVIFOURCC_strh, 0 );			// sunqueen modify
        avi_chunk_STRING_t    *p_strn = (avi_chunk_STRING_t *)AVI_ChunkFind( p_strl, AVIFOURCC_strn, 0 );			// sunqueen modify
        ...
        switch( p_strh->i_type )
        {
            ...
            case( AVIFOURCC_vids ):
            {
                ...
                avi_chunk_vprp_t *p_vprp = (avi_chunk_vprp_t *)AVI_ChunkFind( p_strl, AVIFOURCC_vprp, 0 );			// sunqueen modify
                if( p_vprp )
                {
                ...
                if( fmt.video.i_bits_per_pixel > 0 && fmt.video.i_bits_per_pixel <= 8 )
                {
                    ...
                    {
                        const uint8_t *p_pal = (const uint8_t *)fmt.p_extra;			// sunqueen modify

                        fmt.video.p_palette = (video_palette_t *)calloc( 1, sizeof(video_palette_t) );			// sunqueen modify
...
aviindex:
    ...
    for( i = 0 ; i < p_sys->i_track; i++ )
    {
        ...
        p_strl = (avi_chunk_list_t *)AVI_ChunkFind( p_hdrl, AVIFOURCC_strl, i );			// sunqueen modify
        p_auds = (avi_chunk_strf_auds_t *)AVI_ChunkFind( p_strl, AVIFOURCC_strf, 0 );			// sunqueen modify
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_GET_ATTACHMENTS:
        {
            ...
            *ppp_attach = (input_attachment_t **)calloc( p_sys->i_attachment, sizeof(**ppp_attach));			// sunqueen modify
                              ...
                              avi_entry_t *p_entry )
{
    ...
    {
        p_index->i_max += 16384;
        p_index->p_entry = (avi_entry_t *)realloc_or_free( p_index->p_entry,			// sunqueen modify
                               ...
                               uint64_t *pi_offset )
{
    ...
    avi_chunk_list_t *p_riff = (avi_chunk_list_t *)AVI_ChunkFind( &p_sys->ck_root, AVIFOURCC_RIFF, 0);			// sunqueen modify
    avi_chunk_idx1_t *p_idx1 = (avi_chunk_idx1_t *)AVI_ChunkFind( p_riff, AVIFOURCC_idx1, 0);			// sunqueen modify
    ...
    *pp_idx1 = p_idx1;
     ...
     */
    avi_chunk_list_t *p_movi = (avi_chunk_list_t *)AVI_ChunkFind( p_riff, AVIFOURCC_movi, 0);			// sunqueen modify
...
#define p_stream  p_sys->track[i_stream]
        p_strl = (avi_chunk_list_t *)AVI_ChunkFind( p_hdrl, AVIFOURCC_strl, i_stream );			// sunqueen modify
        p_indx = (avi_chunk_indx_t *)AVI_ChunkFind( p_strl, AVIFOURCC_indx, 0 );			// sunqueen modify
    ...
    for( ;; )
    {
        ...
        else
        {
            ...
            case AVIFOURCC_idx1:
                ...
                {
                    avi_chunk_list_t *p_sysx;
                    p_sysx = (avi_chunk_list_t *)AVI_ChunkFind( &p_sys->ck_root,			// sunqueen modify
                                            AVIFOURCC_RIFF, 1 );
                          ...
                          avi_chunk_list_t *p_riff, avi_chunk_avih_t *p_avih )
{
    ...
    avi_chunk_list_t *p_info = (avi_chunk_list_t *)AVI_ChunkFind( p_riff, AVIFOURCC_INFO, 0 );			// sunqueen modify
    if( !p_info )
        return;
    ...
    {
        avi_chunk_STRING_t *p_strz = (avi_chunk_STRING_t *)AVI_ChunkFind( p_info, p_dsc[i].i_id, 0 );			// sunqueen modify
        if( !p_strz )
            continue;
        ...
        if( *psz_value )
            vlc_meta_Set( p_meta, (vlc_meta_type_t)p_dsc[i].i_type, psz_value );			// sunqueen modify
        free( psz_value );
    }
    ...
    {
        avi_chunk_STRING_t *p_strz = (avi_chunk_STRING_t *)AVI_ChunkFind( p_info, p_extra[i], 0 );			// sunqueen modify
        if( !p_strz )
            continue;
        char *psz_value = FromACP( p_strz->p_str );
        if( !psz_value )
