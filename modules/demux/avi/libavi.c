...
static int AVI_ChunkRead_list( stream_t *s, avi_chunk_t *p_container )
{
    ...
    for( ; ; )
    {
        p_chk = (avi_chunk_t *)xmalloc( sizeof( avi_chunk_t ) );			// sunqueen modify
        memset( p_chk, 0, sizeof( avi_chunk_t ) );
...
#define AVI_READCHUNK_ENTER \
    ...
    if( !( p_read = p_buff = (uint8_t *)malloc(i_read ) ) ) \
    { \
        return VLC_EGENERIC; \
    } \
...
static int AVI_ChunkRead_strf( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    if( !( p_strh = (avi_chunk_t *)AVI_ChunkFind( p_chk->common.p_father, AVIFOURCC_strh, 0 ) ) )			// sunqueen modify
    {
        msg_Err( (vlc_object_t*)s, "malformed avi file" );
        AVI_READCHUNK_EXIT( VLC_EGENERIC );
    }
    ...
    switch( p_strh->strh.i_type )
    {
        case( AVIFOURCC_auds ):
            ...
            p_chk->strf.auds.p_wf = (WAVEFORMATEX *)xmalloc( __MAX( p_chk->common.i_chunk_size, sizeof( WAVEFORMATEX ) ) );			// sunqueen modify
        ...
        case( AVIFOURCC_vids ):
            ...
            p_chk->strf.vids.p_bih = (VLC_BITMAPINFOHEADER *)xmalloc( __MAX( p_chk->common.i_chunk_size,			// sunqueen modify
...
static int AVI_ChunkRead_strd( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    AVI_READCHUNK_ENTER;
    p_chk->strd.p_data = (uint8_t *)xmalloc( p_chk->common.i_chunk_size );			// sunqueen modify
...
static int AVI_ChunkRead_idx1( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    if( i_count > 0 )
    {
        p_chk->idx1.entry = (idx1_entry_t *)xcalloc( i_count, sizeof( idx1_entry_t ) );			// sunqueen modify
...
static int AVI_ChunkRead_indx( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    if( p_indx->i_indextype == AVI_INDEX_OF_CHUNKS && p_indx->i_indexsubtype == 0 )
    {
        ...
        p_indx->idx.std = (indx_std_entry_t *)xcalloc( i_count, sizeof( indx_std_entry_t ) );			// sunqueen modify
    ...
    else if( p_indx->i_indextype == AVI_INDEX_OF_CHUNKS && p_indx->i_indexsubtype == AVI_INDEX_2FIELD )
    {
        ...
        p_indx->i_entriesinuse = i_count;
        p_indx->idx.field = (indx_field_entry_t *)xcalloc( i_count, sizeof( indx_field_entry_t ) );			// sunqueen modify
        for( i = 0; i < i_count; i++ )
        {
    ...
    else if( p_indx->i_indextype == AVI_INDEX_OF_INDEXES )
    {
        ...
        p_indx->i_entriesinuse = i_count;
        p_indx->idx.super = (indx_super_entry_t *)xcalloc( i_count, sizeof( indx_super_entry_t ) );			// sunqueen modify
...
static int AVI_ChunkRead_strz( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    p_strz->p_str = (char *)xmalloc( p_strz->i_chunk_size + 1);			// sunqueen modify
