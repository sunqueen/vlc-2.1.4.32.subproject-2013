...
static int InitTracks( demux_t *p_demux )
{
    ...
    p_sys->track = (mp4_track_t *)calloc( p_sys->i_tracks, sizeof( mp4_track_t ) );			// sunqueen modify
    if( p_sys->track == NULL )
        return VLC_EGENERIC;
    ...
    if( p_sys->b_fragmented )
    {
        ...
        for( uint16_t i = 0; i < p_sys->i_tracks; i++ )
        {
            ...
            p_track->cchunk = (mp4_chunk_t *)calloc( 1, sizeof( mp4_chunk_t ) );			// sunqueen modify
...
#undef SET
                static const struct { uint32_t xa9_type; char *metadata/*[25]*/; } xa9typetoextrameta[] =			// sunqueen modify
                {
        ...
        case DEMUX_GET_TITLE_INFO:
        {
            ...
            *pi_int = 1;
            *ppp_title = (input_title_t **)malloc( sizeof( input_title_t*) );			// sunqueen modify
                                   ...
                                   mp4_track_t *p_demux_track )
{
    ...
    p_demux_track->chunk = (mp4_chunk_t *)calloc( p_demux_track->i_chunk_count,			// sunqueen modify
                                   sizeof( mp4_chunk_t ) );
                                    ...
                                    mp4_track_t *p_demux_track )
{
    ...
    else
    {
        ...
        p_demux_track->p_sample_size =
            (uint32_t *)calloc( p_demux_track->i_sample_count, sizeof( uint32_t ) );			// sunqueen modify
    ...
    for( i_chunk = 0; i_chunk < p_demux_track->i_chunk_count; i_chunk++ )
    {
        ...
        /* allocate them */
        ck->p_sample_count_dts = (uint32_t *)calloc( i_entry, sizeof( uint32_t ) );			// sunqueen modify
        ck->p_sample_delta_dts = (uint32_t *)calloc( i_entry, sizeof( uint32_t ) );			// sunqueen modify
    ...
    if( p_box )
    {
        ...
        for( i_chunk = 0; i_chunk < p_demux_track->i_chunk_count; i_chunk++ )
        {
            ...
            /* allocate them */
            ck->p_sample_count_pts = (uint32_t *)calloc( i_entry, sizeof( uint32_t ) );			// sunqueen modify
            ck->p_sample_offset_pts = (int32_t *)calloc( i_entry, sizeof( int32_t ) );			// sunqueen modify
                                                       ...
                                                       const unsigned cpd_len )
{
    ...
    avcC = (uint8_t *)calloc( length, 1 );			// sunqueen modify
...
static int MP4_frg_GetChunk( demux_t *p_demux, MP4_Box_t *p_chunk, unsigned *i_tk_id )
{
    ...
    *i_tk_id = i_track_ID;
    ...
    ret->p_sample_count_dts = (uint32_t *)calloc( ret->i_sample_count, sizeof( uint32_t ) );			// sunqueen modify
    ret->p_sample_delta_dts = (uint32_t *)calloc( ret->i_sample_count, sizeof( uint32_t ) );			// sunqueen modify

    if( !ret->p_sample_count_dts || !ret->p_sample_delta_dts )
        return VLC_ENOMEM;

    ret->p_sample_count_pts = (uint32_t *)calloc( ret->i_sample_count, sizeof( uint32_t ) );			// sunqueen modify
    if( !ret->p_sample_count_pts )
        return VLC_ENOMEM;

    if( p_trun_data->i_flags & MP4_TRUN_SAMPLE_TIME_OFFSET )
    {
        ret->p_sample_offset_pts = (int32_t *)calloc( ret->i_sample_count, sizeof( int32_t ) );			// sunqueen modify
        if( !ret->p_sample_offset_pts )
            return VLC_ENOMEM;
    }

    ret->p_sample_size = (uint32_t *)calloc( ret->i_sample_count, sizeof( uint32_t ) );			// sunqueen modify
    if( !ret->p_sample_size )
        return VLC_ENOMEM;

    ret->p_sample_data = (uint8_t **)calloc( ret->i_sample_count, sizeof( uint8_t * ) );			// sunqueen modify
    ...
    for( uint32_t i = 0; i < ret->i_sample_count; i++)
    {
        ...
        ret->p_sample_data[i] = (uint8_t *)malloc( len );			// sunqueen modify
