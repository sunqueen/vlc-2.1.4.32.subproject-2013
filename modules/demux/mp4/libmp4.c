...
static int MP4_ReadBox_ftyp( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    {
        uint32_t *tab = p_box->data.p_ftyp->i_compatible_brands = (uint32_t *)			// sunqueen modify
...
static int MP4_ReadBox_tfrf(  stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_tfrf_data->p_tfrf_data_fields = (TfrfBoxDataFields_t *)calloc( p_tfrf_data->i_fragment_count,			// sunqueen modify
...
static int MP4_ReadBox_stra( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_stra->cpd_len > i_read )
        goto error;
    p_stra->CodecPrivateData = (uint8_t *)malloc( p_stra->cpd_len );			// sunqueen modify
...
static int MP4_ReadBox_sidx(  stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_sidx_data->p_items = (MP4_Box_sidx_item_t *)calloc( i_count, sizeof( MP4_Box_sidx_item_t ) );			// sunqueen modify
    uint32_t tmp;
...
static int MP4_ReadBox_trun(  stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_trun->p_samples = (MP4_descriptor_trun_sample_t *)			// sunqueen modify
...
//        uint8_t *psz = p_box->data.p_hdlr->psz_name = malloc( i_read + 1 );
        p_box->data.p_hdlr->psz_name = (unsigned char *)malloc( i_read + 1 );			
		uint8_t *psz = (uint8_t *)p_box->data.p_hdlr->psz_name;
		// sunqueen modify end
...
static int MP4_ReadBox_stts( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_stts->i_sample_count =
        (uint32_t *)calloc( p_box->data.p_stts->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
    p_box->data.p_stts->i_sample_delta =
        (int32_t *)calloc( p_box->data.p_stts->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
...
static int MP4_ReadBox_ctts( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_ctts->i_sample_count =
        (uint32_t *)calloc( p_box->data.p_ctts->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
    p_box->data.p_ctts->i_sample_offset =
        (int32_t *)calloc( p_box->data.p_ctts->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
...
#define es_descriptor p_box->data.p_esds->es_descriptor
    ...
    if( i_type == 0x03 ) /* MP4ESDescrTag */
    {
        ...
        if( es_descriptor.b_url )
        {
            ...
            MP4_GET1BYTE( i_len );
            es_descriptor.psz_URL = (unsigned char *)malloc( i_len + 1 );			// sunqueen modify
    ...
    es_descriptor.p_decConfigDescr =
            (MP4_descriptor_decoder_config_t *)calloc( 1, sizeof( MP4_descriptor_decoder_config_t ));			// sunqueen modify
    ...
    es_descriptor.p_decConfigDescr->p_decoder_specific_info = (uint8_t *)malloc( i_len );			// sunqueen modify
...
static int MP4_ReadBox_hvcC( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_hvcC->i_hvcC > 0 )
    {
        uint8_t * p = p_hvcC->p_hvcC = (uint8_t *)malloc( p_hvcC->i_hvcC );			// sunqueen modify
        if( p )
...
static int MP4_ReadBox_avcC( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_avcC->i_avcC > 0 )
    {
        uint8_t * p = p_avcC->p_avcC = (uint8_t *)malloc( p_avcC->i_avcC );			// sunqueen modify
        if( p )
            memcpy( p, p_peek, i_read );
    }
    ...
    if( p_avcC->i_sps > 0 )
    {
        p_avcC->i_sps_length = (uint16_t *)calloc( p_avcC->i_sps, sizeof( uint16_t ) );			// sunqueen modify
        p_avcC->sps = (uint8_t **)calloc( p_avcC->i_sps, sizeof( uint8_t* ) );			// sunqueen modify
        ...
        for( i = 0; i < p_avcC->i_sps; i++ )
        {
            ...
            p_avcC->sps[i] = (uint8_t *)malloc( p_avcC->i_sps_length[i] );			// sunqueen modify
            if( p_avcC->sps[i] )
    ...
    if( p_avcC->i_pps > 0 )
    {
        p_avcC->i_pps_length = (uint16_t *)calloc( p_avcC->i_pps, sizeof( uint16_t ) );			// sunqueen modify
        p_avcC->pps = (uint8_t **)calloc( p_avcC->i_pps, sizeof( uint8_t* ) );			// sunqueen modify
        ...
        for( i = 0; i < p_avcC->i_pps; i++ )
        {
            ...
            p_avcC->pps[i] = (uint8_t *)malloc( p_avcC->i_pps_length[i] );			// sunqueen modify
            if( p_avcC->pps[i] )
...
static int MP4_ReadBox_dvc1( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_dvc1->i_vc1 > 0 )
    {
        uint8_t *p = p_dvc1->p_vc1 = (uint8_t *)malloc( p_dvc1->i_vc1 );			// sunqueen modify
        if( p )
            memcpy( p, p_peek, i_read );
    }
...
static int MP4_ReadBox_sample_soun( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( i_read > 0 )
    {
        p_box->data.p_sample_soun->p_qt_description = (uint8_t *)malloc( i_read );			// sunqueen modify
...
int MP4_ReadBox_sample_vide( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( i_read > 0 )
    {
        p_box->data.p_sample_vide->p_qt_image_description = (uint8_t *)malloc( i_read );			// sunqueen modify
...
static int MP4_ReadBox_stsz( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_box->data.p_stsz->i_sample_size == 0 )
    {
        p_box->data.p_stsz->i_entry_size =
            (uint32_t *)calloc( p_box->data.p_stsz->i_sample_count, sizeof(uint32_t) );			// sunqueen modify
...
static int MP4_ReadBox_stsc( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_stsc->i_first_chunk =
        (uint32_t *)calloc( p_box->data.p_stsc->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
...
static int MP4_ReadBox_stco_co64( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_co64->i_chunk_offset =
        (uint64_t *)calloc( p_box->data.p_co64->i_entry_count, sizeof(uint64_t) );			// sunqueen modify
...
static int MP4_ReadBox_stss( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_stss->i_sample_number =
        (uint32_t *)calloc( p_box->data.p_stss->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
...
static int MP4_ReadBox_stsh( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_stsh->i_shadowed_sample_number =
        (uint32_t *)calloc( p_box->data.p_stsh->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
...
static int MP4_ReadBox_stdp( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_stdp->i_priority =
        (uint16_t *)calloc( i_read / 2, sizeof(uint16_t) );			// sunqueen modify
...
static int MP4_ReadBox_padb( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_padb->i_reserved1 = (uint16_t *)calloc( count, sizeof(uint16_t) );			// sunqueen modify
    p_box->data.p_padb->i_pad2 = (uint16_t *)calloc( count, sizeof(uint16_t) );			// sunqueen modify
    p_box->data.p_padb->i_reserved2 = (uint16_t *)calloc( count, sizeof(uint16_t) );			// sunqueen modify
    p_box->data.p_padb->i_pad1 = (uint16_t *)calloc( count, sizeof(uint16_t) );			// sunqueen modify
...
static int MP4_ReadBox_elst( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_elst->i_segment_duration =
        (uint64_t *)calloc( p_box->data.p_elst->i_entry_count, sizeof(uint64_t) );			// sunqueen modify
    p_box->data.p_elst->i_media_time =
        (int64_t *)calloc( p_box->data.p_elst->i_entry_count, sizeof(uint64_t) );			// sunqueen modify
    p_box->data.p_elst->i_media_rate_integer =
        (uint16_t *)calloc( p_box->data.p_elst->i_entry_count, sizeof(uint16_t) );			// sunqueen modify
...
static int MP4_ReadBox_cmvd( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( !( p_box->data.p_cmvd->p_data = (uint8_t *)malloc( i_read ) ) )			// sunqueen modify
        MP4_READBOX_EXIT( 0 );
...
static int MP4_ReadBox_cmov( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( !( p_box->data.p_cmov = (MP4_Box_data_cmov_t *)calloc(1, sizeof( MP4_Box_data_cmov_t ) ) ) )			// sunqueen modify
        return 0;
    ...
    if( !( p_data = (uint8_t *)malloc( p_cmvd->data.p_cmvd->i_uncompressed_size ) ) )			// sunqueen modify
        return 0;
...
static int MP4_ReadBox_rdrf( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( i_len > 0 )
    {
        p_box->data.p_rdrf->psz_ref = (char *)malloc( i_len );			// sunqueen modify
...
static int MP4_ReadBox_name( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_name->psz_text = (char *)malloc( p_box->i_size + 1 - 8 ); /* +\0, -name, -size */			// sunqueen modify
...
static int MP4_ReadBox_0xa9xxx( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( i16 > 0 )
    {
        ...
        p_box->data.p_0xa9xxx->psz_text = (char *)malloc( i_length );			// sunqueen modify
    ...
    else
    {
        ...
        if( (i_data_len > 0) && (i_data_tag == ATOM_data) )
        {
            ...
            if( ( i_version == 0x00000001 ) && (i_data_len >= 12 ) )
            {
                ...
                i_data_len -= 12;
                p_box->data.p_0xa9xxx->psz_text = (char *)malloc( i_data_len + 1 );			// sunqueen modify
                if( p_box->data.p_0xa9xxx->psz_text == NULL )
                    MP4_READBOX_EXIT( 0 );
...
static int MP4_ReadBox_chpl( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    for( i = 0; i < p_chpl->i_chapter; i++ )
    {
        ...
        p_chpl->chapter[i].psz_name = (char *)malloc( i_len + 1 );			// sunqueen modify
...
static int MP4_ReadBox_tref_generic( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_box->data.p_tref_generic->i_entry_count > 0 )
        p_box->data.p_tref_generic->i_track_ID = (uint32_t *)calloc( p_box->data.p_tref_generic->i_entry_count, sizeof(uint32_t) );			// sunqueen modify
...
static int MP4_ReadBox_sdtp( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_sdtp->p_sample_table = (uint8_t *)calloc( i_sample_count, 1 );			// sunqueen modify
...
static int MP4_ReadBox_tfra( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_tfra->p_time = (uint32_t *)calloc( i_number_of_entries, size );			// sunqueen modify
    p_tfra->p_moof_offset = (uint32_t *)calloc( i_number_of_entries, size );			// sunqueen modify

    size = 1 + p_tfra->i_length_size_of_traf_num; /* size in [|1, 4|] */
    p_tfra->p_traf_number = (uint8_t *)calloc( i_number_of_entries, size );			// sunqueen modify
    size = 1 + p_tfra->i_length_size_of_trun_num;
    p_tfra->p_trun_number = (uint8_t *)calloc( i_number_of_entries, size );			// sunqueen modify
    size = 1 + p_tfra->i_length_size_of_sample_num;
    p_tfra->p_sample_number = (uint8_t *)calloc( i_number_of_entries, size );			// sunqueen modify
...
static MP4_Box_t *MP4_ReadBox( stream_t *p_stream, MP4_Box_t *p_father )
{
    MP4_Box_t *p_box = (MP4_Box_t *)calloc( 1, sizeof( MP4_Box_t ) ); /* Needed to ensure simple on error handler */			// sunqueen modify
...
MP4_Box_t *MP4_BoxGetSmooBox( stream_t *s )
{
    ...
    p_chunk = (MP4_Box_t *)calloc( 1, sizeof( MP4_Box_t ) );			// sunqueen modify
...
MP4_Box_t *MP4_BoxGetNextChunk( stream_t *s )
{
    ...
    p_tmp_box = (MP4_Box_t *)calloc( 1, sizeof( MP4_Box_t ) );			// sunqueen modify
...
MP4_Box_t *MP4_BoxGetRoot( stream_t *s )
{
    ...
    p_root = (MP4_Box_t *)malloc( sizeof( MP4_Box_t ) );			// sunqueen modify
    if( p_root == NULL )
        return NULL;
