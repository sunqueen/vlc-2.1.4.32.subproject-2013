 ...
 * $Id: aef73888deb28dbff3ee299f6b20ef15623d8ded $
 *
...
int OpenMux( vlc_object_t *p_this )
{
    ...
    p_mux->p_sys = p_sys = (sout_mux_sys_t *)malloc( sizeof( sout_mux_sys_t ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
//        codec->time_base = (AVRational){1, codec->sample_rate};
		...
		time_base.num = 1;
		time_base.den = codec->sample_rate;
    ...
    codec->codec_tag = av_codec_get_tag( p_sys->oc->oformat->codec_tag, (AVCodecID)i_codec_id );			// sunqueen modify
    if( !codec->codec_tag && i_codec_id == AV_CODEC_ID_MP2 )
    {
        i_codec_id = AV_CODEC_ID_MP3;
        codec->codec_tag = av_codec_get_tag( p_sys->oc->oformat->codec_tag, (AVCodecID)i_codec_id );			// sunqueen modify
    }
    codec->codec_id = (AVCodecID)i_codec_id;			// sunqueen modify

    if( p_input->p_fmt->i_extra )
    {
        codec->extradata_size = p_input->p_fmt->i_extra;
        codec->extradata = (uint8_t *)av_malloc( p_input->p_fmt->i_extra );			// sunqueen modify
...
static int IOWrite( void *opaque, uint8_t *buf, int buf_size )
{
    sout_mux_t *p_mux = (sout_mux_t *)opaque;			// sunqueen modify
    int i_ret;
