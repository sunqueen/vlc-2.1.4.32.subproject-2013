...
int OpenDemux( vlc_object_t *p_this )
{
    ...
    p_sys->io_buffer = (uint8_t *)malloc( p_sys->io_buffer_size );			// sunqueen modify
...
//# warning FIXME: implement palette transmission			// sunqueen delete
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            {
                char *psz_start;
                char *psz_buf = (char *)malloc( cc->extradata_size + 1);			// sunqueen modify
                if( psz_buf != NULL )
                {
    ...
    p_sys->tk_pcr = (int64_t *)calloc( p_sys->i_tk, sizeof(*p_sys->tk_pcr) );			// sunqueen modify
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_GET_META:
        {
            static const char names[][11] = {			// sunqueen modify
            ...
            for( unsigned i = 0; i < sizeof(names) / sizeof(*names); i++)
            {
                ...
                if( e != NULL && e->value != NULL && IsUTF8(e->value) )
                    vlc_meta_Set( p_meta, (vlc_meta_type_t)i, e->value );			// sunqueen modify
            }
            return VLC_SUCCESS;
        }
        ...
        case DEMUX_GET_ATTACHMENTS:
        {
            ...
            *ppp_attach = (input_attachment_t **)malloc( sizeof(input_attachment_t**) * p_sys->i_attachments );			// sunqueen modify
...
static int IORead( void *opaque, uint8_t *buf, int buf_size )
{
    demux_t *p_demux = (demux_t *)opaque;			// sunqueen modify
