...
static int Open (vlc_object_t *obj)
{
    ...
    /* Initialization */
    demux_sys_t *sys = (demux_sys_t *)malloc (sizeof (*sys));			// sunqueen modify
    ...
    sys->titlev = (input_title_t **)malloc (n * sizeof (*sys->titlev));			// sunqueen modify
...
{
    stream_t *s = (stream_t *)data;			// sunqueen modify
...
{
    block_t *block = (block_t *)data;			// sunqueen modify
...
static int Demux (demux_t *demux)
{
    ...
    gme_err_t ret = gme_play (sys->emu, 2 * SAMPLES, (short *)block->p_buffer);			// sunqueen modify
    if (ret != NULL)
    {
        block_Release (block);
        msg_Err (demux, "%s", ret);
...
static int Control (demux_t *demux, int query, va_list args)
{
    ...
    switch (query)
    {
        ...
        case DEMUX_GET_TITLE_INFO:
        {
            ...
            *titlev = (input_title_t **)malloc (sizeof (**titlev) * n);			// sunqueen modify
