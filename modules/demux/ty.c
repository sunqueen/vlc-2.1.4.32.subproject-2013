...
static void XdsParse( xds_t *h, uint8_t d1, uint8_t d2 )
{
    ...
    if( d1 >= 0x01 && d1 <= 0x0e )
    {
        const xds_class_t i_class = (const xds_class_t)(( d1 - 1 ) >> 1);			// sunqueen modify
        const int i_type = d2;
...
static void parse_master(demux_t *p_demux)
{
    ...
    p_sys->seq_table = (ty_seq_table_t *)calloc(p_sys->i_seq_table_size, sizeof(ty_seq_table_t));			// sunqueen modify
...
static int get_chunk_header(demux_t *p_demux)
{
    ...
    p_hdr_buf = (uint8_t *)xmalloc(i_num_recs * 16);			// sunqueen modify
                                          ...
                                          int i_num_recs, int *pi_payload_size)
{
    ...
    *pi_payload_size = 0;
    p_hdrs = (ty_rec_hdr_t *)xmalloc(i_num_recs * sizeof(ty_rec_hdr_t));			// sunqueen modify
