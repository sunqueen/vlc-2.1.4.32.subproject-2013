...
static int Open ( vlc_object_t *p_this )
{
    ...
    for( i_max = 0;; )
    {
        ...
        {
            i_max += 500;
            if( !( p_sys->subtitle = (subtitle_t *)realloc_or_free( p_sys->subtitle,			// sunqueen modify
...
static int TextLoad( text_t *txt, stream_t *s )
{
    ...
    txt->i_line_count   = 0;
    txt->i_line         = 0;
    txt->line           = (char **)calloc( i_line_max, sizeof( char * ) );			// sunqueen modify
    if( !txt->line )
        return VLC_ENOMEM;
    ...
    for( ;; )
    {
        ...
        {
            i_line_max += 100;
            txt->line = (char **)realloc_or_free( txt->line, i_line_max * sizeof( char * ) );			// sunqueen modify
            if( !txt->line )
                return VLC_ENOMEM;
        }
    }
                          ...
                          int i_idx )
{
    ...
    for( ;; )
    {
        ...
        psz_text = (char *)malloc( strlen(s) + 1 );			// sunqueen modify
        if( !psz_text )
            return VLC_ENOMEM;
                                 ...
                                 bool b_replace_br )
{
    ...
    for( ;; )
    {
        ...
        psz_text = (char *)realloc_or_free( psz_text, i_old + i_len + 1 + 1 );			// sunqueen modify
        if( !psz_text )
        {
            return VLC_ENOMEM;
        }
                      ...
                      int i_idx )
{
    ...
    for( ;; )
    {
        ...
        psz_text = (char *)malloc( strlen(s) );			// sunqueen modify
        if( !psz_text )
            return VLC_ENOMEM;
...
static int ParseAQT( demux_t *p_demux, subtitle_t *p_subtitle, int i_idx )
{
    ...
    for( ;; )
    {
        ...
        else
        {
            ...
            psz_text = (char *)realloc_or_free( psz_text, i_old + strlen( s ) + 1 );			// sunqueen modify
            if( !psz_text )
                 return VLC_ENOMEM;
            strcat( psz_text, s );
            strcat( psz_text, "\n" );
...
static int ParseMPSub( demux_t *p_demux, subtitle_t *p_subtitle, int i_idx )
{
    ...
    for( ;; )
    {
        ...
        if( strstr( s, "FORMAT" ) )
        {
            ...
            psz_temp = (char *)malloc( strlen(s) );			// sunqueen modify
            if( !psz_temp )
            {
                free( psz_text );
                return VLC_ENOMEM;
...
static int ParseJSS( demux_t *p_demux, subtitle_t *p_subtitle, int i_idx )
{
    ...
    for( ;; )
    {
        ...
        psz_orig = (char *)malloc( strlen( s ) + 1 );			// sunqueen modify
        if( !psz_orig )
            return VLC_ENOMEM;
        psz_text = psz_orig;
    ...
    while( psz_text[ strlen( psz_text ) - 1 ] == '\\' )
    {
        ...
        psz_text = (char *)realloc_or_free( psz_text, i_old + i_len + 1 );			// sunqueen modify
        if( !psz_text )
             return VLC_ENOMEM;
    ...
    psz_orig2 = (char *)calloc( strlen( psz_text) + 1, 1 );			// sunqueen modify
    psz_text2 = psz_orig2;
