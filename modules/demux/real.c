...
static char *StreamReadString2( stream_t *s )
{
    ...
    char *psz_string = (char *)xcalloc( 1, i_length + 1 );			// sunqueen modify
...
static void HeaderINDX( demux_t *p_demux )
{
    ...
    p_sys->p_index = (real_index_t *)calloc( i_index_count + 1, sizeof(*p_sys->p_index) );			// sunqueen modify
    if( !p_sys->p_index )
        return;
...
static int CodecVideoParse( demux_t *p_demux, int i_tk_id, const uint8_t *p_data, int i_data )
{
    ...
    real_track_t *tk = (real_track_t *)malloc( sizeof( *tk ) );			// sunqueen modify
    if( !tk )
    {
        es_format_Clean( &fmt );
        return VLC_ENOMEM;
...
static int CodecAudioParse( demux_t *p_demux, int i_tk_id, const uint8_t *p_data, int i_data )
{
        ...
        fmt.i_codec == VLC_CODEC_ATRAC3 )
    {
        ...
        tk->p_subpackets = (block_t **)			// sunqueen modify
            xcalloc( tk->i_subpackets, sizeof(block_t *) );
        tk->p_subpackets_timecode = (mtime_t *)			// sunqueen modify
