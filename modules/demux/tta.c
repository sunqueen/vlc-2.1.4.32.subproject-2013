...
static int Open( vlc_object_t * p_this )
{
    ...
    fmt.p_extra = p_fullheader = (uint8_t *)malloc( fmt.i_extra );			// sunqueen modify
    if( !p_fullheader )
        goto error;
    ...
    p_sys->pi_seektable = (uint32_t *)calloc( p_sys->i_totalframes, sizeof(uint32_t) );			// sunqueen modify
