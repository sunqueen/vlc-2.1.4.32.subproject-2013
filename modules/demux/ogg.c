...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_GET_ATTACHMENTS:
        {
            ...
            *ppp_attach = (input_attachment_t **)xmalloc( sizeof(input_attachment_t**) * p_sys->i_attachments );			// sunqueen modify
            ...
            if( !p_stream->b_force_backup )
            {
                ...
                if( p_stream->i_headers > 0 )
                    Ogg_ExtractMeta( p_demux, p_stream->fmt.i_codec,
                                     (const uint8_t *)p_stream->p_headers, p_stream->i_headers );			// sunqueen modify
    ...
    while( Ogg_ReadPage( p_demux, &p_ogg->current_page ) == VLC_SUCCESS )
    {
        ...
        if( ogg_page_bos( &p_ogg->current_page ) )
        {
            ...
            while( ogg_page_bos( &p_ogg->current_page ) )
            {
                ...
                p_stream = (logical_stream_t *)malloc( sizeof(logical_stream_t) );			// sunqueen modify
                                ...
                                ogg_packet *p_oggpacket )
{
    ...
    p_stream->fmt.psz_language = (char *)malloc(16);			// sunqueen modify
    ...
    p_stream->fmt.psz_description = (char *)malloc(16);			// sunqueen modify
                                   ...
                                   ogg_packet *p_oggpacket )
{
             ...
             !memcmp( p_oggpacket->packet, "AnxData", 7 ) )
    {
        ...
        {
            uint8_t *p = (uint8_t *)memchr( &p_oggpacket->packet[42], '\r',			// sunqueen modify
                                 p_oggpacket->bytes - 1 );
            if( p && p[0] == '\r' && p[1] == '\n' )
