...
static int Open( vlc_object_t *p_this )
{
    ...
    p_demux->p_sys = p_sys = (demux_sys_t *)malloc( sizeof( demux_sys_t ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
# undef VLC_DVBPSI_DEMUX_TABLE_INIT
    ...
    if( psz_string && *psz_string )
    {
        ...
        else
        {
            ...
            p_sys->buffer = (uint8_t *)xmalloc( p_sys->i_packet_size * p_sys->i_ts_read );			// sunqueen modify
        }
    }
    free( psz_string );
    ...
    case DEMUX_SET_GROUP:
    {
        ...
        else if( i_int < 0 )
        {
            ...
            if( p_list )
            {
                ...
                p_dst->p_values = (vlc_value_t *)calloc( p_list->i_count,			// sunqueen modify
...
static int UserPmt( demux_t *p_demux, const char *psz_fmt )
{
    ...
    /* Dummy PMT */
    prg = (ts_prg_psi_t *)calloc( 1, sizeof( ts_prg_psi_t ) );			// sunqueen modify
    if( !prg )
        goto error;
...
static void PIDInit( ts_pid_t *pid, bool b_psi, ts_psi_t *p_owner )
{
    ...
    if( b_psi )
    {
        ...
        if( !b_old_valid )
        {
            pid->psi = (ts_psi_t *)xmalloc( sizeof( ts_psi_t ) );			// sunqueen modify
            pid->psi->handle = NULL;
        ...
        if( p_owner )
        {
            ts_prg_psi_t *prg = (ts_prg_psi_t *)malloc( sizeof( ts_prg_psi_t ) );			// sunqueen modify
            if( !prg )
                return;
            /* PMT */
            prg->i_version  = -1;
    ...
    {
        pid->psi = NULL;
        pid->es  = (ts_es_t *)calloc( 1, sizeof( ts_es_t ) );			// sunqueen modify
        if( !pid->es )
            return;
...
static iod_descriptor_t *IODNew( int i_data, uint8_t *p_data )
{
    ...
    iod_descriptor_t *p_iod = (iod_descriptor_t *)calloc( 1, sizeof( iod_descriptor_t ) );			// sunqueen modify
    if( !p_iod )
        return NULL;
    ...
    for (int i = 0; i_data > 0 && i < 255; i++)
    {
        ...
        if( i_decoderConfigDescr_length > 13 && IODGetBytes( &i_data, &p_data, 1 ) == 0x05 )
        {
            ...
            {
                dec_descr->p_extra = (uint8_t *)xmalloc( dec_descr->i_extra );			// sunqueen modify
                ...
                if( pE )
                {
                    ...
                    if( pE->i_text_length > 0 )
                    {
                        ...
                        if( psz_text )
                        {
                            ...
                            psz_extra = (char *)xrealloc( psz_extra,			// sunqueen modify
                                ...
                                const dvbpsi_pmt_es_t *p_es )
{
    ...
    else
    {
        for( unsigned i = 0; i < i_page; i++ )
        {
            ...
            {
                p_es = (ts_es_t *)malloc( sizeof(*p_es) );			// sunqueen modify
                if( !p_es )
                    break;
                              ...
                              const dvbpsi_pmt_es_t *p_es )
{
    ...
    pid->es->fmt.psz_language = (char *)malloc( 4 );			// sunqueen modify
    ...
    if( pid->es->fmt.i_extra_languages > 0 )
        pid->es->fmt.p_extra_languages =
            (extra_languages_t *)malloc( sizeof(*pid->es->fmt.p_extra_languages) *			// sunqueen modify
                    pid->es->fmt.i_extra_languages );
    if( pid->es->fmt.p_extra_languages )
    {
        for( int i = 0; i < pid->es->fmt.i_extra_languages; i++ )
        {
            pid->es->fmt.p_extra_languages[i].psz_language = (char *)malloc(4);			// sunqueen modify
