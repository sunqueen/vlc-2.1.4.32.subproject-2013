 ...
 * $Id: 429b9aa3a4460e92e0a12c9108075015cd343358 $
 *
...
static inline int ps_track_fill( ps_track_t *tk, ps_psm_t *p_psm, int i_id )
{
    ...
    {
        tk->fmt.psz_language = (char *)malloc( 4 );			// sunqueen modify
                               ...
                               ps_track_t tk[PS_TK_COUNT], es_out_t *out )
{
    ...
    while( i_es_base + 4 < i_length )
    {
        ...
        if( i_info_length > 0 )
        {
            ...
            es.p_descriptor = (uint8_t *)malloc( i_info_length );			// sunqueen modify
            if( es.p_descriptor )
            {
        ...
        tmp_es = (ps_es_t **)realloc_or_free( p_psm->es, sizeof(ps_es_t *) * (p_psm->i_es+1) );			// sunqueen modify
        if( tmp_es )
        {
            p_psm->es = tmp_es;
            p_psm->es[p_psm->i_es] = (ps_es_t *)malloc( sizeof(ps_es_t) );			// sunqueen modify
