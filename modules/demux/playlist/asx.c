                        ...
                        const char *psz_source_end )
{
    ...
    char *buf = *ppsz_string = (char *)malloc ((len * (1 + !p_sys->b_utf8)) + 1);			// sunqueen modify
    if (buf == NULL)
        return VLC_ENOMEM;
    ...
    else
    {
        ...
        *buf++ = '\0';
        ...
        buf = (char *)realloc (*ppsz_string, buf - *ppsz_string);			// sunqueen modify
        if( buf )
            *ppsz_string = buf;
    }
    return VLC_SUCCESS;
...
static int Demux( demux_t *p_demux )
{
    ...
    if( p_sys->i_data_len < 0 )
    {
        ...
        p_sys->psz_data = (char *)xmalloc( p_sys->i_data_len +1);			// sunqueen modify
        ...
        for( ;; )
        {
            ...
            i_pos += i_read;
            p_sys->i_data_len <<= 1 ;
            p_sys->psz_data = (char *)xrealloc( p_sys->psz_data,			// sunqueen modify
                ...
                if( !strncasecmp( psz_parse, "name", 4 ) )
                {
                    if( ( psz_parse = strcasestr( psz_parse, "\"" ) ) )
                    {
                        ...
                        if( ( psz_parse = strcasestr( psz_parse, "\"" ) ) )
                        {
                            ...
                            psz_string = (char *)xmalloc( i_strlen + 1);			// sunqueen modify
                ...
                if( !strncasecmp( psz_parse, "HREF", 4 ) )
                {
                    if( ( psz_parse = strcasestr( psz_parse, "\"" ) ) )
                    {
                        ...
                        if( ( psz_parse = strcasestr( psz_parse, "\"" ) ) )
                        {
                            ...
                            free( psz_href );
                            psz_href = (char *)xmalloc( i_strlen +1);			// sunqueen modify
