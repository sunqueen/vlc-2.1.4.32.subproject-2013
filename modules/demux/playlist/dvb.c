...
int Import_DVB(vlc_object_t *obj)
{
    ...
    const uint8_t *eol = (const uint8_t *)memchr(peek, '\n', len);			// sunqueen modify
    if (eol == NULL)
        return VLC_EGENERIC;
    len = eol - peek;
...
{
    return strcmp((const char *)k, (const char *)e);			// sunqueen modify
}
...
static const char *ParseFEC(const char *str)
{
     ...
     const struct fec *f = (const struct fec *)bsearch(str, tab, sizeof (tab) / sizeof(tab[0]),			// sunqueen modify
                                   sizeof (tab[0]), cmp);
...
static const char *ParseModulation(const char *str)
{
     ...
     const struct mod *m = (const struct mod *)bsearch(str, tab, sizeof (tab) / sizeof(tab[0]),			// sunqueen modify
                                   sizeof (tab[0]), cmp);
...
static const char *ParseGuard(const char *str)
{
     ...
     const struct guard *g = (const struct guard *)bsearch(str, tab, sizeof (tab) / sizeof(tab[0]),			// sunqueen modify
                                     sizeof (tab[0]), cmp);
