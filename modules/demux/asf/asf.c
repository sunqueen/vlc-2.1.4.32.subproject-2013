...
static int SeekIndex( demux_t *p_demux, mtime_t i_date, float f_pos )
{
    ...
    p_index = (asf_object_index_t *)ASF_FindObject( p_sys->p_root, &asf_object_simple_index_guid, 0 );			// sunqueen modify
...
static int DemuxInit( demux_t *p_demux )
{
    ...
    asf_object_index_t *p_index = (asf_object_index_t *)ASF_FindObject( p_sys->p_root,			// sunqueen modify
                                                  &asf_object_simple_index_guid, 0 );
    const bool b_index = p_index && p_index->i_index_entry_count;

    /* Find the extended header if any */
    asf_object_t *p_hdr_ext = (asf_object_t *)ASF_FindObject( p_sys->p_root->p_hdr,			// sunqueen modify
                                              &asf_object_header_extension_guid, 0 );

    asf_object_language_list_t *p_languages = NULL;
    if( p_hdr_ext )
        p_languages = (asf_object_language_list_t *)ASF_FindObject( p_hdr_ext, &asf_object_language_list, 0 );			// sunqueen modify
    ...
    for( unsigned i_stream = 0; i_stream < p_sys->i_track; i_stream++ )
    {
        ...
        p_sp = (asf_object_stream_properties_t *)ASF_FindObject( p_sys->p_root->p_hdr,			// sunqueen modify
                               &asf_object_stream_properties_guid,
                               i_stream );
        p_esp = NULL;

        tk = p_sys->track[p_sp->i_stream_number] = (asf_track_t *)malloc( sizeof( asf_track_t ) );			// sunqueen modify
        ...
        if( p_hdr_ext )
        {
            ...
            {
                asf_object_t *p_tmp = (asf_object_t *)			// sunqueen modify
    ...
    if( ( p_cd = (asf_object_content_description_t *)ASF_FindObject( p_sys->p_root->p_hdr,			// sunqueen modify
