...
static char *AsfObjectHelperReadString( const uint8_t *p_peek, int i_peek, uint8_t **pp_data, int i_size )
{
    ...
    if( ASF_HAVE(i_size) )
    {
        psz_string = (char *)calloc( i_size/2 + 1, sizeof( char ) );			// sunqueen modify
        if( psz_string )
        {
...
static int  ASF_ReadObject_Header( stream_t *s, asf_object_t *p_obj )
{
    ...
    for( ; ; )
    {
        p_subobj = (asf_object_t *)malloc( sizeof( asf_object_t ) );			// sunqueen modify
...
static int ASF_ReadObject_Index( stream_t *s, asf_object_t *p_obj )
{
    ...
    p_index->index_entry = (asf_index_entry_t *)calloc( p_index->i_index_entry_count,			// sunqueen modify
...
static int ASF_ReadObject_metadata( stream_t *s, asf_object_t *p_obj )
{
    ...
    p_meta->record = (asf_metadata_record_t *)calloc( p_meta->i_record_entries_count,			// sunqueen modify
    ...
    for( i = 0; i < p_meta->i_record_entries_count; i++ )
    {
        ...
        {
            p_record->p_data = (uint8_t *)malloc( i_data );			// sunqueen modify
...
static int ASF_ReadObject_header_extension( stream_t *s, asf_object_t *p_obj )
{
    ...
    if( p_he->i_header_extension_size )
    {
        ...
        p_he->p_header_extension_data =
            (uint8_t *)malloc( p_he->i_header_extension_size );			// sunqueen modify
    ...
    for( ; ; )
    {
        asf_object_t *p_obj = (asf_object_t *)malloc( sizeof( asf_object_t ) );			// sunqueen modify
...
static int ASF_ReadObject_stream_properties( stream_t *s, asf_object_t *p_obj )
{
    ...
    if( p_sp->i_type_specific_data_length )
    {
        ...
        p_sp->p_type_specific_data =
            (uint8_t *)malloc( p_sp->i_type_specific_data_length );			// sunqueen modify
    ...
    if( p_sp->i_error_correction_data_length )
    {
        ...
        p_sp->p_error_correction_data =
            (uint8_t *)malloc( p_sp->i_error_correction_data_length );			// sunqueen modify
...
static int ASF_ReadObject_codec_list( stream_t *s, asf_object_t *p_obj )
{
    ...
    {
        p_cl->codec = (asf_codec_entry_t *)calloc( p_cl->i_codec_entries_count,			// sunqueen modify
        ...
        for( i_codec = 0; i_codec < p_cl->i_codec_entries_count; i_codec++ )
        {
            ...
            {
                p_codec->p_information = (uint8_t *)malloc( p_codec->i_information_length );			// sunqueen modify
...
static int ASF_ReadObject_language_list(stream_t *s, asf_object_t *p_obj)
{
    ...
    {
        p_ll->ppsz_language = (char **)calloc( p_ll->i_language, sizeof( char *) );			// sunqueen modify
                                                      ...
                                                      asf_object_t *p_obj)
{
    ...
    p_esp->pi_stream_name_language = (int *)calloc( p_esp->i_stream_name_count,			// sunqueen modify
                                             sizeof(int) );
    p_esp->ppsz_stream_name = (char **)calloc( p_esp->i_stream_name_count,			// sunqueen modify
                                      sizeof(char*) );
    ...
    if( p_data < &p_peek[i_peek] )
    {
        ...
        p_sp = (asf_object_t *)malloc( sizeof( asf_object_t ) );			// sunqueen modify
                                                     ...
                                                     asf_object_t *p_obj)
{
    ...
    p_ae->pi_stream_number = (int16_t *)calloc( p_ae->i_stream_number_count, sizeof(int) );			// sunqueen modify
                                                 ...
                                                 asf_object_t *p_obj)
{
    ...
    p_sp->pi_priority_flag = (int *)calloc( p_sp->i_priority_count, sizeof(int) );			// sunqueen modify
    p_sp->pi_priority_stream_number =
                             (int *)calloc( p_sp->i_priority_count, sizeof(int) );			// sunqueen modify
                                                        ...
                                                        asf_object_t *p_obj)
{
    ...
    p_ec->ppsz_name  = (char **)calloc( p_ec->i_count, sizeof(char*) );			// sunqueen modify
    p_ec->ppsz_value = (char **)calloc( p_ec->i_count, sizeof(char*) );			// sunqueen modify
    ...
    for( i = 0; i < p_ec->i_count; i++ )
    {
        ...
        {
            /* Byte array */
            static const char hex[/*16*/] = "0123456789ABCDEF";			// sunqueen modify

            p_ec->ppsz_value[i] = (char *)malloc( 2*i_size + 1 );			// sunqueen modify
...
static int ASF_ReadObject_marker(stream_t *s, asf_object_t *p_obj)
{
    ...
    if( p_mk->i_count > 0 )
    {
        p_mk->marker = (asf_marker_t *)calloc( p_mk->i_count,			// sunqueen modify
                              sizeof( asf_marker_t ) );
        if( !p_mk->marker )
            return VLC_ENOMEM;
...
{
    asf_object_root_t *p_root = (asf_object_root_t *)malloc( sizeof( asf_object_root_t ) );			// sunqueen modify
    asf_object_t *p_obj;
    ...
    for( ; ; )
    {
        p_obj = (asf_object_t *)malloc( sizeof( asf_object_t ) );			// sunqueen modify
    ...
    {
        p_root->p_fp = (asf_object_file_properties_t *)ASF_FindObject( p_root->p_hdr,			// sunqueen modify
                                       &asf_object_file_properties_guid, 0 );

        if( p_root->p_fp )
        {
            asf_object_t *p_hdr_ext =
                (asf_object_t *)ASF_FindObject( p_root->p_hdr,			// sunqueen modify
                                &asf_object_header_extension_guid, 0 );
            if( p_hdr_ext )
            {
                int i_ext_stream;

                p_root->p_metadata = (asf_object_metadata_t *)			// sunqueen modify
                ...
                {
                    asf_object_t *p_esp = (asf_object_t *)			// sunqueen modify
                        ASF_FindObject( p_hdr_ext,
