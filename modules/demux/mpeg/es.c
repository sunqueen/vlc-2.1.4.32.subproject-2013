    ...
    *pp_output = NULL;
    ...
    if( p_block_in )
    {
        if( p_sys->codec.b_use_word && !p_sys->b_big_endian && p_block_in->i_buffer > 0 )
        {
            ...
            swab( (char *)p_block_in->p_buffer, (char *)p_block_in->p_buffer, p_block_in->i_buffer );			// sunqueen modify
        }
...
static int A52CheckSync( const uint8_t *p_peek, bool *p_big_endian, int *pi_samples, bool b_eac3 )
{
    ...
    *p_big_endian =  p_peek[0] == 0x0b && p_peek[1] == 0x77;
    if( !*p_big_endian )
    {
        swab( (char *)p_peek, (char *)p_tmp, VLC_A52_HEADER_SIZE );			// sunqueen modify
        p_peek = p_tmp;
    }
