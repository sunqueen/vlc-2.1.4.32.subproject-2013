...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys->i_data = i_size;
    p_sys->p_data = (uint8_t *)malloc( p_sys->i_data );			// sunqueen modify
    if( p_sys->p_data )
...
static int Validate( demux_t *p_demux, const char *psz_ext )
{
    ...
    if( psz_ext && !strcasecmp( psz_ext, "mod" ) && i_peek >= 20 + 15 * 30 )
    {
        ...
        const uint8_t *p = (const uint8_t *)memchr( p_peek, '\0', 20 );			// sunqueen modify
        if( p )
        {
            for( ; p < &p_peek[20]; p++ )
            {
        ...
        for( int i = 0; i < 15; i++ )
        {
            ...
            const uint8_t *p = (const uint8_t *)memchr( &p_sample[0], '\0', 22 );			// sunqueen modify
            if( p )
            {
                for( ; p < &p_sample[22]; p++ )
                {
