...
static int Open( vlc_object_t *p_this )
{
    ...
    /* Set up p_access */
    p_access->p_sys = p_sys = (access_sys_t *)calloc( 1, sizeof( access_sys_t ) );			// sunqueen modify
    if( unlikely(!p_sys ))
        goto error;
    p_sys->vcddev = vcddev;
...
static int Control( access_t *p_access, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case ACCESS_GET_TITLE_INFO:
            ...
            *ppp_title = (input_title_t **)malloc( sizeof(input_title_t **) * p_sys->i_titles );			// sunqueen modify
