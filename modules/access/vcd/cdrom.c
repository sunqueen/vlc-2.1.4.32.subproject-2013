...
vcddev_t *ioctl_Open( vlc_object_t *p_this, const char *psz_dev )
{
     ...
     */
    p_vcddev = (vcddev_t *)malloc( sizeof(*p_vcddev) );			// sunqueen modify
    if( p_vcddev == NULL )
        return NULL;
                        ...
                        int **pp_sectors )
{
    ...
    if( p_vcddev->i_vcdimage_handle != -1 )
    {
        ...
        if( pp_sectors )
        {
            *pp_sectors = (int *)calloc( i_tracks + 1, sizeof(**pp_sectors) );			// sunqueen modify
            if( *pp_sectors == NULL )
                return 0;
                       ...
                       int i_sector, uint8_t *p_buffer, int i_nb, int i_type )
{
    ...
    if( i_type == VCD_TYPE )
        p_block = (uint8_t *)malloc( VCD_SECTOR_SIZE * i_nb );			// sunqueen modify
    else
        p_block = p_buffer;
                         ...
                         vcddev_t *p_vcddev )
{
    ...
    p_pos = (char *)strrchr( psz_dev, '.' );			// sunqueen modify
    if( p_pos && !strcmp( p_pos, ".cue" ) )
    {
        /* psz_dev must be the cue file. Let's assume there's a .bin
         * file with the same filename */
        psz_vcdfile = (char *)malloc( p_pos - psz_dev + 5 /* ".bin" */ );			// sunqueen modify
    ...
    else
    {
        ...
        if( p_pos )
        {
            psz_cuefile = (char *)malloc( p_pos - psz_dev + 5 /* ".cue" */ );			// sunqueen modify
        ...
        switch( i_temp )
        {
            ...
            case 1:
                if( p_vcddev->i_vcdimage_handle == -1 )
                {
                    ...
                    {
                        psz_vcdfile = (char *)malloc( strlen(filename) +			// sunqueen modify
    ...
    while( fgets( line, 1024, cuefile ) && i_tracks < INT_MAX-1 )
    {
        ...
        while( fgets( line, 1024, cuefile ) )
        {
            ...
            int *buf = (int *)realloc (p_sectors, (i_tracks + 1) * sizeof (*buf));			// sunqueen modify
            if (buf == NULL)
                goto error;
            p_sectors = buf;
                        ...
                        const uint8_t *p_buffer, int i_buffer )
{
    ...
    vlc_meta_t **pp_tracks = (vlc_meta_t **)calloc( i_track_last+1, sizeof(*pp_tracks) );			// sunqueen modify
    if( !pp_tracks )
        goto exit;
                       ...
                       uint8_t **pp_buffer, int *pi_buffer )
{
    ...
    uint8_t *p_text = (uint8_t *)calloc( 1, i_text );			// sunqueen modify
    if( !p_text )
        return VLC_EGENERIC;
