                    ...
                    const rar_block_t *hdr, const char *volume_mrl)
{
    ...
    char *name = (char *)calloc(1, name_size + 1);			// sunqueen modify
    if (!name)
        return VLC_EGENERIC;
    ...
    if (!current) {
        if (hdr->flags & RAR_BLOCK_FILE_HAS_PREVIOUS)
            goto exit;
        current = (rar_file_t *)malloc(sizeof(*current));			// sunqueen modify
        if (!current)
            goto exit;
    ...
    /* Append chunks */
    rar_file_chunk_t *chunk = (rar_file_chunk_t *)malloc(sizeof(*chunk));			// sunqueen modify
    if (chunk) {
