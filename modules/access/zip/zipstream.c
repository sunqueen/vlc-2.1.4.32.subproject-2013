...
static int astrcatf( char **ppsz_dest, const char *psz_fmt_src, ... )
{
    ...
    char *psz_out = (char *)realloc( *ppsz_dest, i_len );			// sunqueen modify
    if( !psz_out ) return -1;
                          ...
                          vlc_array_t *p_filenames, vlc_array_t *p_fileinfos )
{
    ...
    {
        char *psz_fileName = (char *)calloc( ZIP_FILENAME_LEN, 1 );			// sunqueen modify
        unz_file_info *p_fileInfo = (unz_file_info *)calloc( 1, sizeof( unz_file_info ) );			// sunqueen modify
...
static int escapeToXml( char **ppsz_encoded, const char *psz_url )
{
    ...
    if( i_num == 0 )
    {
        *ppsz_encoded = (char *)malloc( i_len + 1 );			// sunqueen modify
        memcpy( *ppsz_encoded, psz_url, i_len + 1 );
        return VLC_SUCCESS;
    }

    /* Copy string, replacing invalid characters */
    char *psz_ret = (char *)malloc( i_len + 3*i_num + 2 );			// sunqueen modify
...
static int WriteXSPF( char **pp_buffer, vlc_array_t *p_filenames,
                      const char *psz_zippath )
{
    char *psz_zip = (char *)strrchr( psz_zippath, DIR_SEP_CHAR );			// sunqueen modify
