...
{
    rtp_session_t *session = (rtp_session_t *)malloc (sizeof (*session));			// sunqueen modify
    if (session == NULL)
        return NULL;
...
int rtp_add_type (demux_t *demux, rtp_session_t *ses, const rtp_pt_t *pt)
{
    ...
    rtp_pt_t *ppt = (rtp_pt_t *)realloc (ses->ptv, (ses->ptc + 1) * sizeof (rtp_pt_t));			// sunqueen modify
    if (ppt == NULL)
        return ENOMEM;
                   ...
                   uint32_t ssrc, uint16_t init_seq)
{
    ...
    source = (rtp_source_t *)malloc (sizeof (*source) + (sizeof (void *) * session->ptc));			// sunqueen modify
    if (source == NULL)
        return NULL;
...
rtp_queue (demux_t *demux, rtp_session_t *session, block_t *block)
{
    ...
    if (src == NULL)
    {
        ...
        rtp_source_t **tab;
        tab = (rtp_source_t **)realloc (session->srcv, (session->srcc + 1) * sizeof (*tab));			// sunqueen modify
        if (tab == NULL)
            goto drop;
        session->srcv = tab;
