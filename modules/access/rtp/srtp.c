...
srtp_create (int encr, int auth, unsigned tag_len, int prf, unsigned flags)
{
    ...
    srtp_session_t *s = (srtp_session_t *)malloc (sizeof (*s));			// sunqueen modify
    if (s == NULL)
        return NULL;
...
do_ctr_crypt (gcry_cipher_hd_t hd, const void *ctr, uint8_t *data, size_t len)
{
    ...
    div_t d = div ((int)len, ctrlen);			// sunqueen modify
           ...
           void *out, size_t outlen)
{
    ...
    memset (out, 0, outlen);
    return do_ctr_crypt (prf, iv, (uint8_t *)out, outlen);			// sunqueen modify
}
...
//            memcpy (buf + len, &(uint32_t){ htonl (s->rtp_roc) }, 4);
			temp = htonl (s->rtp_roc);
			memcpy (buf + len, &temp, 4);
			// sunqueen modify end
            len += 4;
        }
