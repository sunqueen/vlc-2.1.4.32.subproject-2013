...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_GET_TITLE_INFO:
            ...
            *ppp_title = (input_title_t **)malloc( sizeof( input_title_t ** ) * p_sys->i_title );			// sunqueen modify
        ...
        case DEMUX_SET_SEEKPOINT:
            ...
            if( p_demux->info.i_title == 0 )
            {
                ...
                if( (unsigned)i >= numargs || DVDNAV_STATUS_OK !=
                           dvdnav_menu_call(p_sys->dvdnav,(DVDMenuID_t)argtab[i]) )			// sunqueen modify
                    return VLC_EGENERIC;
            }
...
static void ESNew( demux_t *p_demux, int i_id )
{
    ...
    else if( tk->fmt.i_cat == AUDIO_ES )
    {
        ...
        if( i_audio >= 0 )
        {
            ...
            if( i_lang != 0xffff )
            {
                tk->fmt.psz_language = (char *)malloc( 3 );			// sunqueen modify
...
{
    demux_sys_t    *p_sys = (demux_sys_t *)p_data;			// sunqueen modify
...
{
    demux_t *p_demux = (demux_t *)p_data;			// sunqueen modify
    demux_sys_t *p_sys = p_demux->p_sys;
