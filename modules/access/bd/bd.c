...
static int Open( vlc_object_t *p_this )
{
    ...
    /* Fill p_demux field */
    p_demux->p_sys = p_sys = (demux_sys_t *)malloc( sizeof(*p_sys) );			// sunqueen modify
    if( !p_sys )
        return VLC_EGENERIC;
...
{
    const bd_mpls_t * const *pp_mpls_a = (const bd_mpls_t *const *)a;			// sunqueen modify
    const bd_mpls_t * const *pp_mpls_b = (const bd_mpls_t *const *)b;			// sunqueen modify
...
static void LoadMpls( demux_t *p_demux, const char *psz_name, int i_id )
{
    ...
    /* */
    bd_mpls_t *p_mpls = (bd_mpls_t *)malloc( sizeof(*p_mpls) );			// sunqueen modify
    if( !p_mpls )
        goto error;
...
static void LoadClpi( demux_t *p_demux, const char *psz_name, int i_id )
{
    ...
    /* */
    bd_clpi_t *p_clpi = (bd_clpi_t *)malloc( sizeof(*p_clpi) );			// sunqueen modify
    if( !p_clpi )
        goto error;
...
static es_out_t *EsOutNew( demux_t *p_demux )
{
    ...
    p_out->p_sys = p_sys = (es_out_sys_t *)malloc( sizeof(*p_sys) );			// sunqueen modify
    if( !p_sys )
    {
        free( p_out );
        return NULL;
