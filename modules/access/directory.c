 ...
 * $Id: 71a446b91d70f28e5e8f463d6cb6740b3a9d1c16 $
 *
...
{
    access_sys_t *p_sys = (access_sys_t *)malloc (sizeof (*p_sys));			// sunqueen modify
    ...
    directory_t *root = (directory_t *)malloc (sizeof (*root));			// sunqueen modify
...
block_t *DirBlock (access_t *p_access)
{
    ...
    if (p_sys->mode != MODE_COLLAPSE)
    {
        ...
        if (p_sys->mode == MODE_NONE)
            goto skip;
#endif
        directory_t *sub = (directory_t *)malloc (sizeof (*sub));			// sunqueen modify
